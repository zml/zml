...
fn vanillaSparseAttention(q: zml.Tensor, kv: zml.Tensor, topk: zml.Tensor, opts: AttentionOptions) zml.Tensor {
    // TODO: add support if sink is null.
    ...
    const sqrt_head_dim = if (opts.scale) |m| m else 1.0 / std.math.sqrt(@as(f32, @floatFromInt(dims.hd)));
    ...
    const sink = opts.attn_sink.insertAxes(0, .{ .batch, .q }).insertAxes(.last, .{.hd}).broad(sink_shape);
...
pub fn sparseAttention(q: zml.Tensor, kv: zml.Tensor, topk: zml.Tensor, parameters: Parameters, opts: AttentionOptions) zml.Tensor {
    return switch (parameters) {
        ...
        .vanilla => vanillaSparseAttention(q, kv, topk, opts),
    };
}
