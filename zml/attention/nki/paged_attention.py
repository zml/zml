...
Entrypoints:
  ...
  paged_attention_decode_2d
    Reads the already-updated paged K/V cache. Each KV stream is shared across
    the GQA query heads that reference that KV head.
...
def power_of_two_shift(value):
    ...
    if value == 512:
        return 9
    assert False, "paged attention expected a power-of-two segment size"
...
def load_static_attention_metadata(
    ...
    """Load prefill/mixed metadata that is reused across Q loops."""
    ...
    return seq_lens_sbuf, query_start_len_sbuf
...
def choose_power_of_two_page_aligned_tile(max_num_pages, page_size, max_tile_tokens):
    """Pick a power-of-two token tile that also divides the block-table row."""
    max_pages = min(max_num_pages, max_tile_tokens // page_size)
    for candidate in (32, 16, 8, 4, 2, 1):
        if candidate <= max_pages and max_num_pages % candidate == 0:
            return candidate * page_size
    assert False, "paged attention could not choose a power-of-two K tile"


def build_k_tile_pages_runtime(
    block_table,
    seq_len_f32,
    seq_idx,
    k_tile_idx,
    ...
    """Build active physical pages for a runtime decode K tile.

    Match the Triton/Pallas contract: treat the public [batch, max_pages] block
    table as a flat pointer and load from seq_idx * max_pages + logical_page.
    The batch row is selected statically by the unrolled decode lane, while the
    on-device K loop contributes only the page offset inside that row.
    """
    block_table_flat = block_table.reshape((1, block_table.shape[0] * max_num_pages))
    physical_pages = nl.ndarray(
        (1, pages_per_k_tile), dtype=block_table.dtype, buffer=nl.sbuf
    )
    page_starts = nl.ndarray((1, pages_per_k_tile), dtype=nl.float32, buffer=nl.sbuf)
    tile_start_pages = nl.ndarray(
        (1, pages_per_k_tile), dtype=nl.float32, buffer=nl.sbuf
    )
    page_offset = nl.ndarray((1, 1), dtype=block_table.dtype, buffer=nl.sbuf)
    tile_start_f32 = nl.ndarray((1, 1), dtype=nl.float32, buffer=nl.sbuf)

    nisa.tensor_scalar(
        dst=page_offset,
        data=k_tile_idx,
        op0=nl.multiply,
        operand0=pages_per_k_tile,
    )
    nisa.dma_copy(
        dst=physical_pages,
        src=block_table_flat.ap(
            pattern=[[1, 1], [1, pages_per_k_tile]],
            offset=seq_idx * max_num_pages,
            scalar_offset=page_offset,
            indirect_dim=1,
        ),
    )

    nisa.tensor_copy(dst=tile_start_f32, src=k_tile_idx)
    nisa.tensor_scalar(
        dst=tile_start_f32,
        data=tile_start_f32,
        op0=nl.multiply,
        operand0=float(k_tile_tokens),
    )
    nisa.tensor_copy(
        dst=tile_start_pages,
        src=nl.broadcast_to(tile_start_f32, tile_start_pages.shape),
    )
    nisa.tensor_tensor(
        dst=page_starts,
        data1=page_offsets,
        data2=tile_start_pages,
        op=nl.add,
    )
    page_live = nl.greater(nl.broadcast_to(seq_len_f32, page_starts.shape), page_starts)
    return nl.where(
        page_live, physical_pages, inactive_pages, dtype=block_table.dtype
    )


def build_k_tile_pages_static(
    block_table,
    active_k_tokens_f32,
    seq_idx,
    k_tile_idx,
    ...
    """Build active physical pages for a statically selected prefill K tile."""
    block_table_flat = block_table.reshape((1, block_table.shape[0] * max_num_pages))
    physical_pages = nl.ndarray(
        (1, pages_per_k_tile), dtype=block_table.dtype, buffer=nl.sbuf
    )
    ...
    page_live = nl.greater(
        nl.broadcast_to(active_k_tokens_f32, page_starts.shape), page_starts
    )
    return nl.where(page_live, physical_pages, inactive_pages, dtype=block_table.dtype)


def build_k_pages_from_page_start_runtime(
    block_table,
    active_k_tokens_f32,
    seq_page_base,
    page_start,
    ...
    """Build active physical pages from a runtime logical-page start.

    Decode segment sizes vary, so the stream cursor is tracked in pages instead
    of fixed K-tile units. Pages beyond the active causal prefix become -1 and
    the later AP loads use oob_mode=skip.
    """
    block_table_flat = block_table.reshape((1, block_table.shape[0] * max_num_pages))
    physical_pages = nl.ndarray(
        (1, pages_per_tile), dtype=block_table.dtype, buffer=nl.sbuf
    )
    block_table_offset = nl.ndarray((1, 1), dtype=nl.uint32, buffer=nl.sbuf)
    page_starts = nl.ndarray((1, pages_per_tile), dtype=nl.float32, buffer=nl.sbuf)
    page_start_f32 = nl.ndarray((1, 1), dtype=nl.float32, buffer=nl.sbuf)
    token_start_f32 = nl.ndarray((1, 1), dtype=nl.float32, buffer=nl.sbuf)
    token_start_pages = nl.ndarray(
        (1, pages_per_tile), dtype=nl.float32, buffer=nl.sbuf
    )

    nisa.tensor_tensor(
        dst=block_table_offset,
        data1=seq_page_base,
        data2=page_start,
        op=nl.add,
    )
    nisa.dma_copy(
        dst=physical_pages,
        src=block_table_flat.ap(
            pattern=[[block_table.shape[0] * max_num_pages, 1], [1, pages_per_tile]],
            offset=0,
            scalar_offset=block_table_offset,
            indirect_dim=1,
        ),
        dge_mode=nisa.dge_mode.swdge,
    )
    nisa.tensor_copy(dst=page_start_f32, src=page_start)
    nisa.tensor_scalar(
        dst=token_start_f32,
        data=page_start_f32,
        op0=nl.multiply,
        operand0=float(page_size),
    )
    nisa.tensor_copy(
        dst=token_start_pages,
        src=nl.broadcast_to(token_start_f32, token_start_pages.shape),
    )
    nisa.tensor_tensor(
        dst=page_starts,
        data1=page_offsets,
        data2=token_start_pages,
        op=nl.add,
    )
    page_live = nl.greater(
        nl.broadcast_to(active_k_tokens_f32, page_starts.shape), page_starts
...
def load_q_group_runtime(q_flat, q_row_base, kv_head, heads_per_kv, d_head, scale):
    """Load all GQA query heads from a runtime compact decode row."""
    q_offset = nl.ndarray((1, 1), dtype=nl.uint32, buffer=nl.sbuf)
    q_sbuf = nl.ndarray((d_head, heads_per_kv), dtype=q_flat.dtype, buffer=nl.sbuf)
    nisa.tensor_scalar(
        dst=q_offset,
        data=q_row_base,
        op0=nl.add,
        operand0=kv_head * heads_per_kv * d_head,
    )
    nisa.dma_copy(
        dst=q_sbuf,
        src=q_flat.ap(
            pattern=[[1, d_head], [d_head, heads_per_kv]],
            offset=0,
            scalar_offset=q_offset,
            indirect_dim=0,
        ),
        dge_mode=nisa.dge_mode.swdge,
    )
...
def store_decode_group_runtime(
    out_flat,
    attn_out_sbuf,
    q_row_base,
    kv_head,
    ...
    """Store one runtime compact decode row back to the flat output."""
    out_offset = nl.ndarray((1, 1), dtype=nl.uint32, buffer=nl.sbuf)
    nisa.tensor_scalar(
        dst=out_offset,
        data=q_row_base,
        op0=nl.add,
    ...
    nisa.dma_copy(
        dst=out_flat.ap(
            pattern=[[d_head, heads_per_kv], [1, d_head]],
            offset=0,
            scalar_offset=out_offset,
            indirect_dim=0,
        ),
        src=attn_out_sbuf,
        dge_mode=nisa.dge_mode.swdge,
...
def stream_decode_segment(
    ...
    """Stream one statically-shaped decode K/V segment."""
    heads_per_kv = q_sbuf.shape[1]
    physical_pages = build_k_pages_from_page_start_runtime(
        block_table,
        active_k_tokens_f32,
        seq_page_base,
        current_page,
    ...
    qk_psum = nl.ndarray((heads_per_kv, K_SEG), dtype=nl.float32, buffer=nl.psum)
    qk_sbuf = nl.ndarray((heads_per_kv, K_SEG), dtype=nl.float32, buffer=nl.sbuf)
    masked_qk = nl.ndarray((heads_per_kv, K_SEG), dtype=nl.float32, buffer=nl.sbuf)
    neg_tile = nl.full(
        (heads_per_kv, K_SEG), NEG_INF, dtype=nl.float32, buffer=nl.sbuf
    )
    ...
    exp_tile = nl.ndarray((heads_per_kv, K_SEG), dtype=nl.float32, buffer=nl.sbuf)
    neg_new_row_max = nl.ndarray((heads_per_kv, 1), dtype=nl.float32, buffer=nl.sbuf)
    exp_t_psum = nl.ndarray((V_SEG, heads_per_kv), dtype=nl.float32, buffer=nl.psum)
    exp_t = nl.ndarray((V_SEG, heads_per_kv), dtype=v_cache.dtype, buffer=nl.sbuf)
    ...
    positions = nl.ndarray((1, K_SEG), dtype=nl.float32, buffer=nl.sbuf)
    page_start_f32 = nl.ndarray((1, 1), dtype=nl.float32, buffer=nl.sbuf)
    token_start_f32 = nl.ndarray((1, 1), dtype=nl.float32, buffer=nl.sbuf)
    token_start_positions = nl.ndarray((1, K_SEG), dtype=nl.float32, buffer=nl.sbuf)
    ...
    nisa.iota(positions, [[1, K_SEG]])
    nisa.tensor_copy(dst=page_start_f32, src=current_page)
    nisa.tensor_scalar(
        dst=token_start_f32,
        data=page_start_f32,
        op0=nl.multiply,
    ...
    nisa.tensor_copy(
        dst=token_start_positions,
        src=nl.broadcast_to(token_start_f32, token_start_positions.shape),
    )
    nisa.tensor_tensor(
        dst=positions,
        data1=positions,
        data2=token_start_positions,
        op=nl.add,
    )
    valid_seq = nl.less(positions, nl.broadcast_to(active_k_tokens_f32, positions.shape))
    masked_qk = nl.where(
    ...
    for v_subtile_idx in nl.affine_range(v_tiles_per_segment):
        v_start = v_subtile_idx * V_SEG
        nisa.nc_transpose(
            dst=exp_t_psum,
            data=exp_tile[:, nl.ds(v_start, V_SEG)],
        )
...
def compute_decode_kv_head_dynamic_k_loop(
    q_flat,
    out_flat,
    k_cache,
    v_cache,
    ...
    """Run one decode row/KV head with a runtime K-segment loop."""
    V_SEG = min(nl.tile_size.pmax, K_SEG)
    pages_per_segment = K_SEG // page_size
    pages_per_v_tile = V_SEG // page_size
    v_tiles_per_segment = K_SEG // V_SEG
    page_offsets = make_page_offsets(pages_per_segment, page_size)
    ...
    attn_out_sbuf = nl.ndarray(
        (heads_per_kv, d_head), dtype=out_flat.dtype, buffer=nl.sbuf
    )
    reset_row_max = nl.full(
        (heads_per_kv, 1), NEG_INF, dtype=nl.float32, buffer=nl.sbuf
    )
    reset_sum_row = nl.full((heads_per_kv, 1), 0.0, dtype=nl.float32, buffer=nl.sbuf)
    reset_running_out = nl.full(
        (heads_per_kv, d_head), 0.0, dtype=nl.float32, buffer=nl.sbuf
    )
    seq_len_plus_segment = nl.ndarray((1, 1), dtype=active_seq_len.dtype, buffer=nl.sbuf)
    segment_count = nl.ndarray((1, 1), dtype=active_seq_len.dtype, buffer=nl.sbuf)
    current_page = nl.full((1, 1), 0, dtype=nl.uint32, buffer=nl.sbuf)

    nisa.tensor_copy(dst=row_max, src=reset_row_max)
    nisa.tensor_copy(dst=sum_row, src=reset_sum_row)
    nisa.tensor_copy(dst=running_out, src=reset_running_out)
    nisa.tensor_scalar(
        dst=seq_len_plus_segment,
        data=active_seq_len,
        op0=nl.add,
        operand0=K_SEG - 1,
    )
    nisa.tensor_scalar(
        dst=segment_count,
        data=seq_len_plus_segment,
        op0=nl.right_shift,
        operand0=power_of_two_shift(K_SEG),
    )
    segment_count_reg = nisa.register_alloc(None)
    nisa.register_load(dst=segment_count_reg, src=segment_count)

    q_sbuf = load_q_group_runtime(
        q_flat,
        q_row_base,
        kv_head,
        heads_per_kv,
    ...
    for _ in nl.dynamic_range(segment_count_reg):
        stream_decode_segment(
            k_cache,
            v_cache,
            block_table,
            q_sbuf,
        ...
        nisa.tensor_scalar(
            ...
            data=current_page,
            op0=nl.add,
            operand0=pages_per_segment,
        )

    one_row = nl.full((heads_per_kv, 1), 1.0, dtype=nl.float32, buffer=nl.sbuf)
    zero_out = nl.full((heads_per_kv, d_head), 0.0, dtype=nl.float32, buffer=nl.sbuf)
    active_row = nl.greater(
        nl.broadcast_to(active_seq_len_f32, (heads_per_kv, 1)),
        nl.full((heads_per_kv, 1), 0.0, dtype=nl.float32, buffer=nl.sbuf),
    )
    safe_sum_row = nl.where(active_row, sum_row, one_row, dtype=nl.float32)
    safe_running_out = nl.where(
        nl.broadcast_to(active_row, running_out.shape),
        running_out,
        zero_out,
        dtype=nl.float32,
    )
...
def paged_attention_decode_2d(
    ...
    Decode expects paged_kv_cache_update to have produced the current cache row
    as a functional input. This matches the Triton/Pallas contract: attention
    reads cache and does not update it. Do not add the prefill q_loop_reg
    pattern here: decode has no compact Q-block sweep to hide from tracing.
    """
    ...
    q_flat = q.reshape((num_tokens * num_kv_heads * heads_per_kv * d_head, 1))
    out_flat = out.reshape((num_tokens * num_kv_heads * heads_per_kv * d_head, 1))

    seq_lens_flat = seq_lens.reshape((1, batch_size))
    seq_lens_sbuf = nl.ndarray((1, batch_size), dtype=seq_lens.dtype, buffer=nl.sbuf)
    query_start_len_sbuf = nl.ndarray(
        (1, batch_size + 1), dtype=query_start_len.dtype, buffer=nl.sbuf
    )
    nisa.dma_copy(dst=seq_lens_sbuf, src=seq_lens_flat[0:1, 0:batch_size])
    nisa.dma_copy(
        dst=query_start_len_sbuf,
        src=query_start_len.reshape((1, batch_size + 1))[0:1, 0 : batch_size + 1],
    )
    active_rows = query_start_len_sbuf[nl.ds(0, 1), nl.ds(batch_size, 1)]
    K_SEG = choose_power_of_two_page_aligned_tile(
        max_num_pages, page_size, nl.tile_size.gemm_moving_fmax
    )

    for kv_head in nl.sequential_range(num_kv_heads):
        for seq_idx in nl.static_range(num_tokens):
            seq_len = seq_lens_sbuf[nl.ds(0, 1), nl.ds(seq_idx, 1)]
            seq_len_f32 = nl.ndarray((1, 1), dtype=nl.float32, buffer=nl.sbuf)
            active_seq_len = nl.ndarray((1, 1), dtype=seq_lens.dtype, buffer=nl.sbuf)
            active_seq_len_f32 = nl.ndarray((1, 1), dtype=nl.float32, buffer=nl.sbuf)
            active_rows_f32 = nl.ndarray((1, 1), dtype=nl.float32, buffer=nl.sbuf)
            q_idx_f32 = nl.full((1, 1), float(seq_idx), dtype=nl.float32, buffer=nl.sbuf)
            zero_i32 = nl.full((1, 1), 0, dtype=seq_lens.dtype, buffer=nl.sbuf)
            zero_f32 = nl.full((1, 1), 0.0, dtype=nl.float32, buffer=nl.sbuf)
            q_row_base = nl.full(
                (1, 1),
                seq_idx * num_kv_heads * heads_per_kv * d_head,
                dtype=nl.uint32,
                buffer=nl.sbuf,
            )
            seq_page_base = nl.full(
                (1, 1),
                seq_idx * max_num_pages,
                dtype=nl.uint32,
                buffer=nl.sbuf,
            )

            nisa.tensor_copy(dst=seq_len_f32, src=seq_len)
            nisa.tensor_copy(dst=active_rows_f32, src=active_rows)
            active_row = nl.greater(active_rows_f32, q_idx_f32)
            nisa.tensor_copy(
                dst=active_seq_len,
                src=nl.where(active_row, seq_len, zero_i32, dtype=seq_lens.dtype),
            )
            nisa.tensor_copy(
                dst=active_seq_len_f32,
                src=nl.where(active_row, seq_len_f32, zero_f32, dtype=nl.float32),
            )
            compute_decode_kv_head_dynamic_k_loop(
                q_flat,
                out_flat,
                k_cache,
                v_cache,
...
def build_grouped_q_metadata(
    ...
    q_rows_global = nl.ndarray((q_group_tile, 1), dtype=nl.float32, buffer=nl.sbuf)
    q_rows_local = nl.ndarray((q_group_tile, 1), dtype=nl.float32, buffer=nl.sbuf)
    ...
    for head_group in nl.affine_range(heads_per_kv):
        group_offset = head_group * q_per_head_tile
        nisa.tensor_copy(
            dst=q_rows_global[nl.ds(group_offset, q_per_head_tile), 0:1],
            src=base_positions,
        )
    ...
    nisa.tensor_tensor(
        dst=q_rows_global,
        data1=q_rows_global,
        data2=nl.broadcast_to(local_q_start_f32, q_rows_global.shape),
        op=nl.add,
    )
    valid_q_row = nl.less(
        q_rows_global, nl.broadcast_to(q_end_f32, q_rows_global.shape)
    )
    nisa.tensor_tensor(
        dst=q_rows_local,
        data1=q_rows_global,
        data2=nl.broadcast_to(q_start_f32, q_rows_local.shape),
        op=nl.subtract,
    )
    nisa.tensor_tensor(
        dst=q_positions_abs,
        data1=q_rows_local,
        data2=nl.broadcast_to(context_len, q_positions_abs.shape),
        op=nl.add,
    )
...
def paged_attention_2d(
    ...
    seq_lens_sbuf, query_start_len_sbuf = load_static_attention_metadata(
        seq_lens,
        query_start_len,
        batch_size,
    )
    ...
    for kv_head in nl.affine_range(num_kv_heads):
        for seq_idx in nl.affine_range(batch_size):
            ...
            while q_loop_reg:
                ...
                seq_len_f32 = nl.ndarray((1, 1), dtype=nl.float32, buffer=nl.sbuf)
                q_len_f32 = nl.ndarray((1, 1), dtype=nl.float32, buffer=nl.sbuf)
                context_len = nl.ndarray((1, 1), dtype=nl.float32, buffer=nl.sbuf)
                current_q_start_f32 = nl.ndarray(
                    (1, 1), dtype=nl.float32, buffer=nl.sbuf
                )
                local_q_offset = nl.ndarray((1, 1), dtype=nl.float32, buffer=nl.sbuf)
                q_tile_rows = nl.ndarray((1, 1), dtype=nl.float32, buffer=nl.sbuf)
                q_tile_limit = nl.full(
                    (1, 1),
                    float(q_per_head_tile),
                    dtype=nl.float32,
                    buffer=nl.sbuf,
                )
                q_tile_query_end = nl.ndarray(
                    (1, 1), dtype=nl.float32, buffer=nl.sbuf
                )
                causal_k_end_unclamped = nl.ndarray(
                    (1, 1), dtype=nl.float32, buffer=nl.sbuf
                )
                causal_k_end = nl.ndarray((1, 1), dtype=nl.float32, buffer=nl.sbuf)
                nisa.tensor_copy(dst=seq_len_f32, src=seq_len)
                nisa.tensor_tensor(
                    dst=q_len_f32,
                    data1=q_end_f32,
                ...
                nisa.tensor_copy(dst=current_q_start_f32, src=current_q_start)
                nisa.tensor_tensor(
                    dst=local_q_offset,
                    data1=current_q_start_f32,
                    data2=q_start_f32,
                    op=nl.subtract,
                )
                nisa.tensor_tensor(
                    dst=q_tile_rows,
                    data1=q_tokens_remaining,
                    data2=q_tile_limit,
                    op=nl.minimum,
                )
                nisa.tensor_tensor(
                    dst=q_tile_query_end,
                    data1=local_q_offset,
                    data2=q_tile_rows,
                    op=nl.add,
                )
                nisa.tensor_tensor(
                    dst=causal_k_end_unclamped,
                    data1=context_len,
                    data2=q_tile_query_end,
                    op=nl.add,
                )
                nisa.tensor_tensor(
                    dst=causal_k_end,
                    data1=causal_k_end_unclamped,
                    data2=seq_len_f32,
                    op=nl.minimum,
                )
                ...
                for current_k_tile in nl.static_range(num_k_tiles):
                    physical_pages = build_k_tile_pages_static(
                        block_table,
                        causal_k_end,
                        seq_idx,
                        current_k_tile,
                    ...
                    nisa.iota(key_positions_f32, [[1, K_TILE]])
                    nisa.tensor_scalar(
                        dst=key_positions_f32,
                        data=key_positions_f32,
                        op0=nl.add,
                        operand0=current_k_tile * K_TILE,
                    )
                    valid_k = nl.less(
                        key_positions_f32,
                        nl.broadcast_to(causal_k_end, key_positions_f32.shape),
                    )
                    ...
                    combined_mask = nl.logical_and(
                        combined_mask,
                        nl.broadcast_to(valid_k, qk_sbuf.shape),
                    )
                    masked_qk = nl.where(
                        combined_mask,
                        qk_sbuf,
