...
//! `zml/attention/triton_attention.zig`.
...
pub const KernelUnifiedAttention2dPtr = struct {
    pub const Kernel = tri.Kernel(Config2D, .{
        ...
        .inputs = &.{
            ...
            "output_stride_0_ptr",      "output_stride_1_ptr",    "qq_bias_stride_0_ptr",
            "stride_k_cache_0_ptr",     "stride_k_cache_1_ptr",   "stride_k_cache_2_ptr",
            "stride_v_cache_0_ptr",     "stride_v_cache_1_ptr",   "stride_v_cache_2_ptr",
            "query_start_len_ptr",      "num_seqs_ptr",
        },
        .outputs = &.{"output"},
        .run = run,
    });
    fn run(b: *Builder, cfg: Config2D) tri.FinishError!void {
...
pub const KernelUnifiedAttention3dPtr = struct {
    pub const Kernel = tri.Kernel(Config3D, .{
    ...
    fn run(b: *Builder, cfg: Config3D) tri.FinishError!void {
        const a = try b.declareArgs(.{
            .query_ptr = .{ .ptr = cfg.q_dtype },
            .key_cache_ptr = .{ .ptr = cfg.kv_dtype },
            .value_cache_ptr = .{ .ptr = cfg.kv_dtype },
...
pub const ReduceSegmentsPtr = struct {
    pub const Kernel = tri.Kernel(ConfigReduce, .{
    ...
    fn run(b: *Builder, cfg: ConfigReduce) tri.FinishError!void {
        const a = try b.declareArgs(.{
            .segm_output_ptr = .{ .ptr = .f32 },
            .segm_max_ptr = .{ .ptr = .f32 },
            .segm_expsum_ptr = .{ .ptr = .f32 },
