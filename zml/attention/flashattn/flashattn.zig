...
pub const Prefill = struct {
    pub const custom_call_name = "flashattn_prefill";
...
pub const PrefillVarlen = struct {
    pub const custom_call_name = "flashattn_prefill_varlen";
    const Wrapped = Wrapper(@This(), .runInner);
...
pub fn register(platform: zml.Platform) !void {
    try Prefill.register(platform);
    try PrefillVarlen.register(platform);
