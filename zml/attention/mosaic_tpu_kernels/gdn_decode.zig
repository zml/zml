...
pub const Kernel = zml.kernel.mosaic_tpu.Kernel(Cfg, .{
    .name = "gdn_decode",
    // Program-id arg (pid) is part of the func.func signature but Mosaic
    // synthesizes it from `iteration_bounds`, so it isn't a `tpu_custom_call`
    // operand. (Pallas does the same.)
    .inputs = &.{ "q", "k", "v", "g", "beta", "s_in" },
    .outputs = &.{ "o", "s_out" },
    .run = run,
});
