...
pub const CustomCallRegistry = opaque {
    const inner = InnerMixin(pjrt.CustomCallRegistry).inner;

    pub const RegisterFfiOptions = struct {
        traits: pjrt.RegisterHandlerTraits = @enumFromInt(0),
    };
    ...
    pub fn registerFfi(
        ...
        self.inner().registerFfi(api, target_name, platform_name, func, options);
    }
};
