module @zml attributes {mhlo.num_partitions = 4 : i32, mhlo.num_replicas = 1 : i32} {
  func.func @main(%arg0: tensor<1x512x10x64xf32> {mhlo.layout_mode = "default", mhlo.sharding = "{replicated}"}, %arg1: tensor<1x512x10x64xf32> {mhlo.layout_mode = "default", mhlo.sharding = "{replicated}"}, %arg2: tensor<1x512x10x64xf32> {mhlo.layout_mode = "default", mhlo.sharding = "{replicated}"}, %arg3: tensor<512x512xf32> {mhlo.layout_mode = "default", mhlo.sharding = "{replicated}"}) -> (tensor<1x512x10x64xf32> {mhlo.layout_mode = "default", mhlo.sharding = "{replicated}"}) {
    ...
    %0 = stablehlo.broadcast_in_dim %cst_1, dims = [] : (tensor<f32>) -> tensor<1x512x10x64xf32>
    %1 = stablehlo.multiply %arg1, %0 : tensor<1x512x10x64xf32>
    %2 = stablehlo.dot_general %arg0, %1, batching_dims = [0, 2] x [0, 2], contracting_dims = [3] x [3], precision = [DEFAULT, DEFAULT] : (tensor<1x512x10x64xf32>, tensor<1x512x10x64xf32>) -> tensor<1x10x512x512xf32>
    ...
    %5 = stablehlo.reduce(%4 init: %cst_0) across dimensions = [3] {operandSegmentSizes = dense<1> : tensor<2xi32>} : (tensor<1x10x512x512xf32>, tensor<f32>) -> tensor<1x10x512xf32>
     reducer(%arg4: tensor<f32>, %arg5: tensor<f32>)  {
      %21 = stablehlo.maximum %arg5, %arg4 : tensor<f32>
      stablehlo.return %21 {operandSegmentSizes = dense<1> : tensor<1xi32>} : tensor<f32>
    }
    ...
    %13 = stablehlo.reduce(%12 init: %cst) across dimensions = [3] {operandSegmentSizes = dense<1> : tensor<2xi32>} : (tensor<1x10x512x512xf32>, tensor<f32>) -> tensor<1x10x512xf32>
     reducer(%arg4: tensor<f32>, %arg5: tensor<f32>)  {
      %21 = stablehlo.add %arg5, %arg4 : tensor<f32>
      stablehlo.return %21 {operandSegmentSizes = dense<1> : tensor<1xi32>} : tensor<f32>
    }
    ...
    %19 = stablehlo.dot_general %18, %arg2, batching_dims = [0, 1] x [0, 2], contracting_dims = [3] x [1], precision = [DEFAULT, DEFAULT] : (tensor<1x10x512x512xf32>, tensor<1x512x10x64xf32>) -> tensor<1x10x512x64xf32>
    %20 = stablehlo.transpose %19, dims = [0, 2, 1, 3] : (tensor<1x10x512x64xf32>) -> tensor<1x512x10x64xf32>
    return %20 : tensor<1x512x10x64xf32>
  }
}
