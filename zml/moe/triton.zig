...
fn callFusedMoe(
    ...
    const b_unflattened = b;
    const b_ = flattenGateUpLocalForKernel(b_unflattened);
    const kernel_output_shape = kernelOutputShape(b_unflattened, output_shape);
    ...
    return reshapeKernelOutputForLogicalShape(b_unflattened, output_shape, kernel_output_shape, kernel_output);
}
...
fn alignBlockSize(topk_ids: Tensor, num_experts: i64, block_size_m: i64) struct { Tensor, Tensor, Tensor } {
...

    {
        ...
        sorted_token_ids = outs.sorted_token_ids;
        expert_ids = outs.expert_ids;
        num_tokens_post_padded = outs.num_tokens_post_pad;
        cumsums = outs.cumsum;
    }
