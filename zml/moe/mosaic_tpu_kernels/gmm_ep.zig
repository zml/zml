...
pub fn run(b: *mtt.Builder, cfg: Cfg) mtt.FinishError!void {
    ...
    if (!use_1d_pipeline) @panic("gmm_ep currently requires a single K tile, a single N tile, and size_m > tile_m");
    const lhs_3d = b.memRefReshape(a.lhs, &.{ @divExact(cfg.size_m, cfg.size_lhs_sublane), cfg.size_lhs_sublane, cfg.size_k });
    const out_3d = b.memRefReshape(a.out, &.{ @divExact(cfg.size_m, cfg.size_lhs_sublane), cfg.size_lhs_sublane, cfg.alignedN() });
    try pipeline.emitPipeline(b, &.{ lhs_3d, a.rhs, out_3d }, &.{ a.partial_out, a.acc, a.metadata_group_id, a.metadata_m_offset }, .{
...
pub const Kernel = struct {
    pub const name: [:0]const u8 = "gmm_v2-g_1-m_8-k_128-act_None-n_128-tm_8-tk_128-tn_128";
    ...
    pub fn emit(allocator: std.mem.Allocator, cfg: Config) ![:0]const u8 {
        ...
        const fn_name = try std.fmt.allocPrint(
            allocator,
            "gmm_v2-g_{d}-m_{d}-k_{d}-act_None-n_{d}-tm_{d}-tk_{d}-tn_{d}",
            .{ cfg.size_group, cfg.size_m, cfg.size_k, cfg.size_n, cfg.tile_m, cfg.tile_k, cfg.tile_n },
        );
...
pub const SWEEPS: []const harness.Sweep(Kernel.Config) = &.{
    ...
    .{ .label = "m16_two_gm", .cfg = override(.{ .size_m = 16 }) },
    .{ .label = "m32_tm16", .cfg = override(.{ .size_m = 32, .tile_m = 16 }) },
    .{ .label = "m64_tm16", .cfg = override(.{ .size_m = 64, .tile_m = 16 }) },
    .{ .label = "m64_tm32", .cfg = override(.{ .size_m = 64, .tile_m = 32 }) },
    .{ .label = "g2_m16", .cfg = override(.{ .size_m = 16, .size_group = 2, .size_lhs_group = 2 }) },
    .{ .label = "g4_m32", .cfg = override(.{ .size_m = 32, .size_group = 4, .size_lhs_group = 4 }) },
    .{ .label = "k256_tk256", .cfg = override(.{ .size_k = 256, .tile_k = 256 }) },
    .{ .label = "k512_tk512", .cfg = override(.{ .size_k = 512, .tile_k = 512 }) },
    .{ .label = "n256_tn256", .cfg = override(.{ .size_n = 256, .tile_n = 256 }) },
    .{ .label = "lhs_f32", .cfg = override(.{ .lhs_dtype = .f32, .out_dtype = .f32 }) },
    .{ .label = "rhs_f32", .cfg = override(.{ .rhs_dtype = .f32 }) },
    .{ .label = "all_f32", .cfg = override(.{ .lhs_dtype = .f32, .rhs_dtype = .f32, .out_dtype = .f32 }) },
    .{ .label = "qwen35_gmm2_downproj", .cfg = override(.{ .size_m = 64, .size_k = 512, .size_n = 2048, .size_group = 256, .size_lhs_group = 256, .tile_m = 64, .tile_k = 512, .tile_n = 2048 }) },
    .{ .label = "qwen35_gmm1_noact_shape", .cfg = override(.{ .size_m = 64, .size_k = 2048, .size_n = 1024, .size_group = 256, .size_lhs_group = 256, .tile_m = 64, .tile_k = 2048, .tile_n = 1024 }) },
    .{ .label = "gemma4_gmm2_downproj", .cfg = override(.{ .size_m = 64, .size_k = 704, .size_n = 2816, .size_group = 128, .size_lhs_group = 128, .tile_m = 64, .tile_k = 704, .tile_n = 2816 }) },
    .{ .label = "gemma4_gmm1_noact_shape", .cfg = override(.{ .size_m = 64, .size_k = 2816, .size_n = 1408, .size_group = 128, .size_lhs_group = 128, .tile_m = 64, .tile_k = 2816, .tile_n = 1408 }) },
};
