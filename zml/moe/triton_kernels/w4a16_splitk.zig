...
fn run(b: *tri.Builder, cfg: Cfg) tri.FinishError!void {
    ...
    // Preserve the non-split kernel's expert-local L2 traversal while using Z
    // for independent K partitions.
    ...
    });
    {
        ...
        // The caller requires K to be an exact multiple of BLOCK_K and caps
        // SPLIT_K at the number of K tiles.
...
test "emit routed W4A16 split-K kernel" {
    const ttir = try Kernel.emit(std.testing.allocator, .{ .BLOCK_M = 4, .GROUP_M = 4 });
