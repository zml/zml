...
fn toCommonActivation(activation: triton.Parameters.ActivationMode) common.ActivationMode {
    ...
    pub const InitOptions = union(Backend) {
        ...
        pub fn fromBackend(backend: Backend, num_experts_per_tok: ?u32, activation: triton.Parameters.ActivationMode) InitOptions {
            return switch (backend) {
                ...
                .mosaic_tpu => .{ .mosaic_tpu = .{ .num_experts_per_tok = num_experts_per_tok.?, .activation = toCommonActivation(activation) } },
            };
        }
    };
