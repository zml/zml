...
//     pub fn init(allocator: std.mem.Allocator, io: std.Io, memory: *const Memory, pool: *mem.DynamicBufferPool, shape: Shape, sharding: Sharding, buffer: *Buffer) !MemoryWriter {
//         return switch (memory.platform.target) {
//             .cuda, .rocm => .{ .direct = try .init(allocator, io, memory, pool, shape, sharding, buffer) },
//             .tpu, .neuron, .cpu => .{ .buffered = try .init(allocator, io, memory, shape, sharding, buffer) },
//         };
//     }
