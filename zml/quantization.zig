...
pub fn quantizeInput(quantization: Quantization, input: Tensor, axis: Shape.Tag, platform: *const Platform) ?QuantizedInput {
    ...
    return switch (quantization.scheme) {
        ...
        .fp8_block32 => switch (platform.target) {
            // Block-scaled MMA consumes activation scales as E8M0.
            .cuda => quantizeBlockMxFp8(input, axis, 32),
            .rocm => if (platform_mod.rocm.computeCapability(platform)) |capability| switch (capability.architecture()) {
...
/// Scales divide the contracting dimension by `block_size`.
pub fn quantizeBlockFp8(x: Tensor, axis: anytype, block_size: u32, dtype: DataType) QuantizedInput {
    ...
    stdx.debug.assert(@mod(x.dim(axis), block_size) == 0, "block FP8 activation width must be divisible by {d}, got {f}", .{ block_size, x.shape() });
...
/// Quantize activation blocks with power-of-two E8M0 scales.
/// Scales preserve the input axes with the contracting dimension divided by `block_size`.
pub fn quantizeBlockMxFp8(x: Tensor, axis: anytype, block_size: u32) QuantizedInput {
    stdx.debug.assert(@mod(x.dim(axis), block_size) == 0, "block FP8 activation width must be divisible by {d}, got {f}", .{ block_size, x.shape() });
    const fp8_max: f32 = 448.0;
    const grouped = x.convert(.f32).splitAxis(axis, .{ .fp8_ks = -1, .fp8_block = block_size });
    const amax = grouped.abs().max(.fp8_block).maximum(.scalar(1e-10, .f32));
    const bits = amax.scale(1.0 / fp8_max).bitCast(.u32);
    // Round a nonzero mantissa up to the next exponent to avoid clipping the block maximum.
    const inexact = bits.logical(.AND, .scalar(0x7fffff, .u32)).cmp(.NE, .scalar(0, .u32)).convert(.u32);
    const scale = bits.shiftRightLogical(.scalar(23, .u32)).add(inexact).shiftLeft(.scalar(23, .u32)).bitCast(.f32);
    const values = grouped.div(scale.broad(grouped.shape()))
        .clamp(.scalar(-fp8_max, .f32), .scalar(fp8_max, .f32))
        .convert(.f8e4m3fn)
        .reshape(x.shape().withDtype(.f8e4m3fn));
    return .{
        .values = values,
        .scales = scale.convert(.f8e8m0).reshape(x.shape().setDim(axis, @divExact(x.dim(axis), block_size)).withDtype(.f8e8m0)),
        .global_scale = null,
    };
}
