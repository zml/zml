...
pub const TensorReader = struct {
    ...
    pub fn initBorrowedPositional(io: std.Io, tensor: Tensor, file: std.Io.File) TensorReader {
        return initBorrowedPositionalMode(io, tensor, file, .batch_and_resume);
    }

    /// Creates a positional-only borrowed reader for a backend that either
    /// fills the complete scatter list in one call or fails.
    pub fn initBorrowedPositionalExact(
        io: std.Io,
        tensor: Tensor,
        file: std.Io.File,
    ) TensorReader {
        return initBorrowedPositionalMode(io, tensor, file, .single_exact_fill);
    }

    fn initBorrowedPositionalMode(
        io: std.Io,
        tensor: Tensor,
        file: std.Io.File,
        mode: PositionalReadMode,
    ) TensorReader {
...
test "TensorReader borrowed exact-fill mode passes more than IOV_MAX buffers once" {
    ...
    var reader = TensorReader.initBorrowedPositionalExact(io, .{
        .file_uri = "unused",
        .name = "tensor",
        .shape = .init(.{len}, .u8),
        .offset = 7,
