...
pub const PhysicalMesh = struct {
    ...
    fn assignCoords(node: *PhysicalNode, path: *[Shape.MAX_RANK]usize, depth: usize) !void {
        switch (node.*) {
            .leaf => |*d| {
                ...
                for (path[0..depth]) |c_| coords.appendAssumeCapacity(c_);
                d.coords = coords;
            },
            .branch => |*b| {
                for (b.children, 0..) |*child, i| {
    ...
    pub fn neuron(allocator: std.mem.Allocator, platform_devices: []const PlatformDevice) !Tree {
        if (comptime @hasDecl(c, "ZML_RUNTIME_NEURON")) {
            ...
            var axis_tags: stdx.BoundedArray(PhysicalAxisTag, Shape.MAX_RANK) = try .init(0);
            var axis_sizes: stdx.BoundedArray(usize, Shape.MAX_RANK) = try .init(0);
            var axis_geometries: stdx.BoundedArray(AxisGeometry, Shape.MAX_RANK) = try .init(0);
