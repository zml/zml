...
pub const Mesh = struct {
    ...
    pub fn init(topology: anytype) Mesh {
        ...
        if (self.rank() == 0) {
            stdx.debug.panic("Mesh must have at least one tagged axis defined, got: {any}", .{topology});
        }

        if (!self.topology.isFullyTagged()) {
            stdx.debug.panic("Mesh must be fully tagged, got: {any}", .{topology});
        }
    ...
    pub fn format(
        ...
        try writer.print("Mesh(topology={f} rank={} numDevices={})", .{ self.topology, self.rank(), self.numDevices() });
    }
};
...
pub const Sharding = struct {
        ...
        for (0..shape.rank()) |dim_idx| {
            ...
            if (part_spec == .axis) {
                ...
                if (mesh.topology.hasTag(mesh_axis_tag) == null) {
                    ...
                    log.debug("Partitioning axis '{s}' not found in mesh {f}. Treating dimension {s} as replicated.", .{
    ...
    pub fn writeShardingRepresentation(self: Sharding, writer: anytype) !void {
        ...
        try writer.writeAll("]<=[");
        try writer.print("{}", .{self.mesh.numDevices()});
