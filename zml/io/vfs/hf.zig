...
test "HF download authorization is restricted to the exact trusted origin" {
    try std.testing.expect(HF.trustedDownloadOrigin(try .parse("https://huggingface.co/model")));
    try std.testing.expect(HF.trustedDownloadOrigin(try .parse("https://huggingface.co:443/model")));
    try std.testing.expect(!HF.trustedDownloadOrigin(try .parse("http://huggingface.co/model")));
    try std.testing.expect(!HF.trustedDownloadOrigin(try .parse("https://huggingface.co:444/model")));
    try std.testing.expect(!HF.trustedDownloadOrigin(try .parse("https://cdn-lfs.huggingface.co/model")));
    try std.testing.expect(!HF.trustedDownloadOrigin(try .parse("https://huggingface.co.evil/model")));
}
...
pub const HF = struct {
    ...
    pub fn backend(self: *HF) Backend {
        return .{
            ...
            .read_hints = .{
                ...
                .high_latency = true,
                .exact_positional_fill = true,
                .exact_read_timing = true,
            },
    ...
    fn resolveDownloadUrl(self: *HF, repo: Repo, out_buffer: []u8) ![]const u8 {
        var initial_url_buffer: [16 * 1024]u8 = undefined;
        const url = try std.fmt.bufPrint(&initial_url_buffer, API.LFS_FILE_URL_TEMPLATE, repo);
        var uri: std.Uri = try .parse(url);
        var redirect_buffers: [2][16 * 1024]u8 = undefined;
        ...
        while (true) {
            {
                var req = try self.client.request(.HEAD, uri, .{
                    ...
                    .headers = .{
                        .accept_encoding = .{ .override = "identity" },
                        .authorization = if (trustedDownloadOrigin(uri)) self.authorization else .omit,
                    },
                });
                defer req.deinit();
                ...
                const res = try req.receiveHead(&head_buffer);
                switch (res.head.status.class()) {
                    .success => return std.fmt.bufPrint(out_buffer, "{f}", .{uri}),
                    .redirect => {
                        if (redirect_count >= 8) return error.TooManyRedirects;
                        const location = res.head.location orelse return error.InvalidRedirect;
                        const redirect_buffer = &redirect_buffers[redirect_count % redirect_buffers.len];
                        if (location.len > redirect_buffer.len) return error.InvalidRedirect;
                        @memcpy(redirect_buffer[0..location.len], location);
                        var redirect_slice: []u8 = redirect_buffer;
                        uri = try uri.resolveInPlace(location.len, &redirect_slice);
                        redirect_count += 1;
                    },
                    else => {
                        log.err("Failed to resolve download URL for {f}: status={}", .{ uri, res.head.status });
                        return error.RequestFailed;
                    },
                }
            }
    ...
    fn trustedDownloadOrigin(uri: std.Uri) bool {
    ...
    fn performRead(self: *HF, handle: *Handle, data: []const []u8, offset: u64) !usize {
        ...
        const authorization: std.http.Client.Request.Headers.Value = if (trustedDownloadOrigin(uri))
            self.authorization
        else
            .omit;
        ...
        var attempt: usize = 0;
        while (true) {
            switch (try self.performReadAttempt(uri, url, authorization, data, handle.size, offset, read_size, attempt == 0)) {
