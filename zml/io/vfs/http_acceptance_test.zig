...
const MockServer = struct {
    ...
    fn init(
        ...
        return .{
            ...
            .head_redirect = opts.head_redirect,
        };
    }
    ...
    fn handleConnection(self: *MockServer, stream: std.Io.net.Stream, io: std.Io) !void {
        ...
        const is_object = std.mem.eql(u8, request.head.target, "/object");
        const is_resolved = std.mem.eql(u8, request.head.target, "/resolved");
        if (!is_object and !is_resolved) {
        ...
        switch (request.head.method) {
            .HEAD => {
                ...
                if (is_object) if (self.head_redirect) |location| {
                    try request.respond("", .{
            ...
            .GET => {
                if ((self.head_redirect != null) != is_resolved) return error.UnexpectedTarget;
...
test "generic HTTP resolves a relative HEAD redirect before its single range GET" {
    const allocator = std.testing.allocator;
    const io = std.testing.io;
    const object = "redirected object";

    var server = try MockServer.init(io, object, .{ .head_redirect = "/resolved" });
    ...
    defer client.deinit();
    var http = try HTTP.init(allocator, io, &client, .http);
    defer http.deinit();
    ...
    var output: [object.len]u8 = undefined;
    try std.testing.expectEqual(
        object.len,
        try file.readPositional(http_io, &.{&output}, 0),
    );
    try std.testing.expectEqualStrings(object, &output);
...
test "generic HTTP bounds HEAD redirect loops" {
    const allocator = std.testing.allocator;
    const io = std.testing.io;
    const object = "loop";

    var server = try MockServer.init(io, object, .{ .head_redirect = "/object" });
    var server_group: std.Io.Group = .init;
    try startMockServer(&server, &server_group, io);
    var server_joined = false;
    defer cleanupMockServer(&server, &server_group, io, server_joined);
    ...
    try std.testing.expectError(
        error.Unexpected,
        std.Io.Dir.openFile(.cwd(), http.io(), path, .{ .mode = .read_only }),
    );

    server_group.cancel(io);
    server_joined = true;
    try server.check();
    try std.testing.expectEqual(@as(usize, 9), server.head_requests.load(.acquire));
    try std.testing.expectEqual(@as(usize, 0), server.get_requests.load(.acquire));
}
