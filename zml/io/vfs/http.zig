...
pub const HTTP = struct {
    ...
    fn dirStatFile(userdata: ?*anyopaque, dir: std.Io.Dir, sub_path: []const u8, _: std.Io.Dir.StatFileOptions) std.Io.Dir.StatFileError!std.Io.File.Stat {
        const self: *HTTP = @alignCast(@fieldParentPtr("base", VFSBase.as(userdata)));
        var resolved_url: [16 * 1024]u8 = undefined;
        const metadata = self.resolveFile(dir, sub_path, &resolved_url) catch return std.Io.Dir.StatFileError.Unexpected;
    ...
    fn dirOpenFile(userdata: ?*anyopaque, dir: std.Io.Dir, sub_path: []const u8, _: std.Io.File.OpenFlags) std.Io.File.OpenError!std.Io.File {
        ...
        var resolved_url: [16 * 1024]u8 = undefined;
        const metadata = self.resolveFile(dir, sub_path, &resolved_url) catch return std.Io.File.OpenError.Unexpected;

        const idx, const handle = self.openHandle() catch return std.Io.File.OpenError.Unexpected;
        handle.* = Handle.init(self.allocator, .file, metadata.url, metadata.size) catch return std.Io.File.OpenError.Unexpected;
    ...
    fn resolveFile(
        ...
        const full_url = try std.fmt.bufPrint(&initial_url_buffer, "{s}://{s}", .{ scheme, url });
        ...
        while (true) {
            {
                ...
                switch (res.head.status.class()) {
                    ...
                    .success => {
                        const size = res.head.content_length orelse return error.MissingContentLength;
                        const final_url = try std.fmt.bufPrint(resolved_url, "{f}", .{uri});
                        return .{ .size = size, .url = final_url };
                    },
                    .redirect => {
    ...
    fn performRead(self: *HTTP, handle: *Handle, data: []const []u8, offset: u64) !usize {
        ...
        const url = handle.uri;
        const uri: std.Uri = try .parse(url);

        var attempt: usize = 0;
        while (true) {
            switch (try self.performReadAttempt(uri, url, data, handle.size, offset, read_size, attempt == 0)) {
