...
pub fn nativeBlockScaledDotPreparedLocal(
    ...
    return switch (Compiler.current().platform.target) {
        .cuda => xlaPreparedBlockScaledDot(prepared, output_shape),
        .rocm => tritonPreparedBlockScaledDotLocal(prepared, output_shape),
        else => unreachable,
    };
}
...
fn xlaPreparedBlockScaledDot(prepared: PreparedDot, output_shape: Shape) Tensor {
...
fn xlaBlockScaledDot(lhs: Tensor, rhs_fn: Tensor, rhs_scale_fn: Tensor, output_shape: Shape) Tensor {
    return xlaPreparedBlockScaledDot(prepareBlockScaledDot(lhs, rhs_fn, rhs_scale_fn), output_shape);
}
...
fn blockScaledDot(
    ...
    return ops.manualComputationWithContext(
