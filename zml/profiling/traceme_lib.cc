#include "zml/profiling/traceme.h"
...
struct RocmRoctxRangeBackend {
  explicit RocmRoctxRangeBackend(const std::string& name)
    ...
    const int level = RangePush()(name.c_str());
    active = level >= 0;
  }

  ~RocmRoctxRangeBackend() {
    if (active) {
      RangePop()();
    }
  }
  ...
  static bool IsEnabled() {
    return RangePush() != nullptr && RangePop() != nullptr;
  }

 private:
  using RangePushFn = decltype(&roctxRangePushA);
  using RangePopFn = decltype(&roctxRangePop);

  static RangePushFn RangePush() {
    return reinterpret_cast<RangePushFn>(dlsym(RTLD_DEFAULT, "roctxRangePushA"));
  }

  static RangePopFn RangePop() {
    return reinterpret_cast<RangePopFn>(dlsym(RTLD_DEFAULT, "roctxRangePop"));
  }
...
#else
struct RocmRoctxRangeBackend {
  explicit RocmRoctxRangeBackend(const std::string&) {}
  static bool IsEnabled() { return false; }
};
#endif

#if defined(__APPLE__)
struct MacOsSignpostRangeBackend {
  explicit MacOsSignpostRangeBackend(const std::string& name)
      : log(Logger()), signpost_id(os_signpost_id_generate(log)) {
    os_signpost_interval_begin(log, signpost_id, "zml.scope", "%{public}s",
                               name.c_str());
  }

  ~MacOsSignpostRangeBackend() {
    os_signpost_interval_end(log, signpost_id, "zml.scope", "%{public}s",
                             "");
  }
 ...
 private:
  static os_log_t Logger() {
    static os_log_t log =
        os_log_create("ai.zml", OS_LOG_CATEGORY_POINTS_OF_INTEREST);
    return log;
  }
...
#else
struct MacOsSignpostRangeBackend {
  explicit MacOsSignpostRangeBackend(const std::string&) {}
  static bool IsEnabled() { return false; }
};
#endif
...
struct zml_traceme {
  ...
  RocmRoctxRangeBackend rocm_backend;
  MacOsSignpostRangeBackend macos_backend;
};
...
extern "C" bool zml_traceme_enabled(void) {
  return tsl::profiler::TraceMe::Active() ||
         CudaNvtxRangeBackend::IsEnabled() ||
         RocmRoctxRangeBackend::IsEnabled() ||
         MacOsSignpostRangeBackend::IsEnabled();
}
