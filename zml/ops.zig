...
/// On-device while loop. Executes `body_fn` repeatedly while `cond_fn` returns true.
///
/// `init` is a tuple of Tensors representing the initial loop state.
/// `cond_fn` receives the state as separate arguments and must return a scalar boolean Tensor (pred).
/// `body_fn` receives the state as separate arguments and returns a tuple of the same types as `init`.
///
/// Returns the final state after the loop terminates (same types as `init`).
pub fn whileLoop(
    init: anytype,
    ...
    // Build MLIR types for block arguments (one per init tensor)
    var block_types: [N]*const mlir.Type = undefined;
    var block_locs: [N]*const mlir.Location = undefined;
    inline for (0..N) |i| {
        block_types[i] = mlir.rankedTensorType(init[i].dims(), mlirx.Type.fromDType(mlir_ctx, init[i].dtype()));
        block_locs[i] = loc;
    }

    // --- Condition block ---
    const cond_block = mlir.Block.init(&block_types, &block_locs);
    {
        CompilationContext.current().pushBlock(cond_block);
        defer CompilationContext.current().popBlock();

        var cond_args: @TypeOf(init) = undefined;
        const scope = CompilationContext.current().currentScope();
        inline for (0..N) |i| {
            cond_args[i] = Tensor.fromShape(init[i].shape());
            scope.id_to_argument.put(scope.arena.allocator(), cond_args[i].id, i) catch unreachable;
        }

        const pred = @call(.auto, cond_fn, cond_args);
        _ = dialects.stablehlo.returns(mlir_ctx, &.{pred.value()}, loc).appendTo(cond_block);
    }

    // --- Body block ---
    const body_block = mlir.Block.init(&block_types, &block_locs);
    var result: @TypeOf(init) = undefined;
    {
        CompilationContext.current().pushBlock(body_block);
        defer CompilationContext.current().popBlock();

        var body_args: @TypeOf(init) = undefined;
        const scope = CompilationContext.current().currentScope();
        inline for (0..N) |i| {
            body_args[i] = Tensor.fromShape(init[i].shape());
            scope.id_to_argument.put(scope.arena.allocator(), body_args[i].id, i) catch unreachable;
        }

        const body_result = @call(.auto, body_fn, body_args);

        var result_values: [N]*const mlir.Value = undefined;
        inline for (0..N) |i| {
            result_values[i] = body_result[i].value();
        }
        _ = dialects.stablehlo.returns(mlir_ctx, &result_values, loc).appendTo(body_block);
    }

    // --- Create the while operation ---
    var init_values: [N]*const mlir.Value = undefined;
    var result_types: [N]*const mlir.Type = undefined;
    inline for (0..N) |i| {
        init_values[i] = init[i].value();
        result_types[i] = block_types[i];
    }

    const while_op = dialects.stablehlo.while_loop(
        mlir_ctx,
        &init_values,
        &result_types,
        cond_block,
    ...
    inline for (0..N) |i| {
        result[i] = Tensor._result(init[i].shape(), while_op.result(i));
    }
