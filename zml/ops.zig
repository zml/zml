...
fn customCallShardyInfo(
    allocator: std.mem.Allocator,
    ctx: *CompilationContext,
    shape: Shape,
) struct {
    local_shape: Shape,
    sharding_attr: *const mlir.Attribute,
} {
    log.err("SHARDY SHAPE: {f}", .{shape});
    const sharding = ctx.partitioning.selectSharding(shape) catch unreachable;
    log.err("SHARDY SHARDINg: {f}", .{sharding});
    const attr_info = ctx.partitioning.tensorShardingAttr(allocator, shape, sharding) catch unreachable;
    stdx.debug.assert(attr_info != null and std.mem.eql(u8, attr_info.?.name, "sdy.sharding"), "customCall with shardy_manual_computation expects explicit sdy sharding for shape {f}", .{shape});

    const placement = sharding_.Placement.init(sharding, shape) catch unreachable;
    const local_shape = placement.shards.get(0).shape;
    const sharding_attr = mlir.Attribute.parse(ctx.mlir_ctx, attr_info.?.attr) catch unreachable;
    return .{
...
fn customCallInternal(target_name: [:0]const u8, inputs: []const Tensor, outputs: []const Shape, metadata: anytype, opts: CustomCallOptions) []Tensor {
    ...
    const op = if (opts.shardy_manual_computation and ctx.partitioning.partitioner == .shardy) blk: {
        const input_shardings = arena.allocator().alloc(*const mlir.Attribute, inputs.len) catch unreachable;
        const output_shardings = arena.allocator().alloc(*const mlir.Attribute, outputs.len) catch unreachable;
        ...
        for (input_shapes, 0..) |input_shape, i| {
            const info = customCallShardyInfo(arena.allocator(), ctx, input_shape);
            local_input_shapes[i] = info.local_shape;
            input_shardings[i] = info.sharding_attr;
        }
        for (outputs, 0..) |output_shape, i| {
            const info = customCallShardyInfo(arena.allocator(), ctx, output_shape);
            local_output_shapes[i] = info.local_shape;
            output_shardings[i] = info.sharding_attr;
        }
        ...
        break :blk mlir.Operation.make(ctx.mlir_ctx, "sdy.manual_computation", .{
            ...
            .blocks = &.{manual_block},
            .attributes = &.{
                .named(ctx.mlir_ctx, "in_shardings", mlir.arrayAttribute(ctx.mlir_ctx, input_shardings)),
                .named(ctx.mlir_ctx, "out_shardings", mlir.arrayAttribute(ctx.mlir_ctx, output_shardings)),
            },
            .verify = true,
            .location = .unknown(ctx.mlir_ctx),
        }).appendTo(CompilationContext.current().currentScope().block);
