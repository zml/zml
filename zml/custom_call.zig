...
pub const CustomCallOptions = struct {
    ...
    register_ffi_options: pjrt.CustomCallRegistry.RegisterFfiOptions = .{
...
pub fn custom_call(
    ...
    const registry = ffi_.?.customCallRegistry();
    const ffi_func = proxy(custom_op, opts);
    const target_name = "callback_" ++ op_name;

    registry.registerFfi(pjrt_api, target_name, @tagName(platform.target), &ffi_func, opts.register_ffi_options) catch unreachable;
        ...
        pub fn proxy(call_frame: *ffi.CallFrame) callconv(.C) ?*ffi.Error {
            ...
            inline for (1..callback_args.len) |i| {
                ...
                if (platform.target == .cpu) {
                    zml_buffer = Buffer.asViewOfHostBuffer(platform, HostBuffer.fromBytes(ffi_buffer_shape, ffi_buffer.data.asPtr()[0..ffi_buffer_shape.byteSize()]));
                } else {
                    zml_buffer = Buffer.asViewOfDeviceBuffer(platform, getShape(ffi_buffer), null, ffi_buffer.data.asPtr());
                }
                if (opts.copy_inputs_to_host_pinned and platform.target != .cpu) {
                    zml_buffer = zml_buffer.copyToMemory(platform, .host_pinned, .{ .should_await_blocking = true }) catch unreachable;
                }
            ...
            for (0..call_frame.results.len) |i| {
                ...
                if (platform.target == .cpu) {
                    user_ctx.results[i] = Buffer.asViewOfHostBuffer(platform, HostBuffer.fromBytes(ffi_buffer_shape, ffi_buffer.data.asPtr()[0..ffi_buffer_shape.byteSize()]));
                } else {
                    user_ctx.results[i] = Buffer.asViewOfDeviceBuffer(platform, getShape(ffi_buffer), null, ffi_buffer.data.asPtr());
                }
            }
