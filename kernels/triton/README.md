...
2. [`zml.kernel.triton.Kernel(Config, spec)` — the declarative form](#zmlkerneltritonkernelconfig-spec--the-declarative-form)
3. [Calling kernels from a ZML model — `K.Kernel.call(...)`](#calling-kernels-from-a-zml-model--kkernelcall)
...
pub const MyKernel = struct {
    pub const Cfg = struct { BLOCK: i32 = 64 };
    pub const Kernel = tri.Kernel(Cfg, .{
        .name = "my_kernel",
        .inputs = &.{ "x_ptr", "y_ptr", "n" },
        .outputs = &.{"out"},
        .run = run,
    });
    fn run(b: *tri.Builder, cfg: Cfg) tri.FinishError!void {
        ...
        const y = x.mul(2.0);
        b.storeOpts(a.out_ptr.addPtr(offs), y, .{ .mask = mask });
    }
};
```
...
1. **`pub const Cfg`** — config type, an inline `struct { ... }` of
   comptime/runtime parameters. Use field defaults for sensible defaults.
2. **`pub const Kernel = tri.Kernel(Cfg, .{ .name, .inputs, .outputs, .run })`** —
...
3. **`fn run(b, cfg)`** — the kernel body. First call `try b.declareArgs(.{…})`
   to create the `tt.func` op and bind a struct of `Value`s named after the
   spec's fields. The rest of `run` builds the body via `b.*` helpers. No
   string return, no allocator, no manual `tt.return` — the factory
   terminates, verifies, and serializes for you.
...
the names in `b.declareArgs(...)` (which are the **TTIR-level** arg names).
By convention they often do — pick whatever reads best at the boundary.
...
const out = MyKernel.Kernel.call(
    .{ .x_ptr = x_tensor, .y_ptr = y_tensor, .n = n_tensor },  // Inputs
    .{ .out = x_tensor.shape() },                                // Outputs (Shapes)
    .{
        .cfg = .{ .BLOCK = 1024 },
...
`Kernel.call` returns a `Results` struct (one `Tensor` per output name);
read fields by name (`out.out`, `align_outs.sorted_token_ids`, …).

---

## `zml.kernel.triton.Kernel(Config, spec)` — the declarative form

`zml.kernel.triton.Kernel(Config, spec)` is a comptime function returning a
kernel type. `Config` is the per-call config type (used both for the
generated `Inputs`/`Outputs`/`CallOpts` and for the typed `run` signature);
`spec` is a `Spec(Config)` literal:

| Field       | Type                                              | Meaning                                                             |
|-------------|---------------------------------------------------|---------------------------------------------------------------------|
| `.name`     | `[:0]const u8`                                    | Kernel name (used as the `tt.func` symbol).                         |
| `.inputs`   | `[]const [:0]const u8`                            | Field names for the call-site `Inputs` struct (fields are `Tensor`). Order must match the `tt.func` arg order for input pointers. |
| `.outputs`  | `[]const [:0]const u8`                            | Field names for the `Outputs` (struct of `Shape`) and `Results` (struct of `Tensor`). Order must match the `tt.func` arg order for output pointers — XLA's lowering passes inputs first, then outputs. |
| `.run`      | `*const fn (*Builder, Config) FinishError!void`   | Typed function pointer to the kernel body. Wrong signature → compile error here, not at first call. |

Wrong arity, missing field, typo'd name on the call site → compile error
at the call site (because `Inputs` / `Outputs` are real struct types, not
tuples).

`run` is responsible for declaring the kernel's args (via
`b.declareArgs(...)`) and emitting the body. It emits ops via `b.*` helpers
and never returns a string; `Kernel.emit(...)` does the
terminate-verify-serialize dance after `run` returns.

---

## Calling kernels from a ZML model — `K.Kernel.call(...)`

Inside a model's `forward(...)`, drop in a Triton kernel alongside regular
ZML ops with `K.Kernel.call(...)`:
...
pub fn forward(self: Model, x: zml.Tensor, y: zml.Tensor) zml.Tensor {
    ...
    const out = MyKernel.Kernel.call(
        .{ .x_ptr = x_pre, .y_ptr = y, .n = n_tensor },      // Inputs (by name)
        .{ .out = x_pre.shape() },                            // Outputs (Shapes)
        .{
...
|----------------|------------------|-------------------------------------------------------------------------|
| `inputs`       | `Kernel.Inputs`  | Struct of `Tensor`, one field per name in `spec.inputs`.                |
| `outputs`      | `Kernel.Outputs` | Struct of `Shape`, one field per name in `spec.outputs`.                |

`Kernel.CallOpts` (typed struct, all fields named):

| Field                     | Type                  | Meaning                                  |
|---------------------------|-----------------------|------------------------------------------|
| `.cfg`                    | `Kernel.Config`       | Per-call kernel config.                  |
| `.grid`                   | `[3]i32`              | Triton launch grid `(grid_x, grid_y, grid_z)`. |
| `.num_warps`              | `i32`                 | Triton launch param.                     |
| `.num_stages`             | `i32`                 | Triton launch param.                     |
| `.output_operand_aliases` | `[]const ...` (default `&.{}`) | For in-place updates (still indexed by output position — `.{ .output_index = 0, .operand_index = 1 }`). |
| `.debug`                  | `bool` (default false)| Emit `tt.debug = true` on the custom call. |

Returns `Kernel.Results` — a struct of `Tensor` named like `spec.outputs`.
Read by name: `out.out`, `attn_output.segm_max`, etc.
...
> **Why named `Inputs` / `Outputs`.** The previous form took
> `anytype` tuples and matched by position; misordering 27 args (typical
> in `unified_attention.zig`) was a silent corruption hazard. With
> generated structs (`Inputs = StructOf(spec.inputs, Tensor)`,
> `Outputs = StructOf(spec.outputs, Shape)`), Zig refuses unknown fields,
> missing fields, and wrong-typed values at the callsite.
...
The declarative `zml.kernel.triton.Kernel(Config, spec)` form covers most
cases. For existing code, dynamic-arity arg lists, or escape-hatch scenarios
where you want to drive the IR construction yourself, the lower-layer
`Builder.build` API is still available:
