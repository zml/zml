...
fn kernelUnifiedAttentionSparseMla(
    ...
    const VALUE_RANK_PADDED: i64 = @intCast(std.math.ceilPowerOfTwoAssert(usize, @intCast(VALUE_RANK)));
    ...
    const offs_value = k.arange(0, VALUE_RANK_PADDED, .i32);
    const value_mask = offs_value.lt(@as(i32, @intCast(VALUE_RANK)));
    ...
    const acc_init = k.zeros(&.{ BLOCK_M, VALUE_RANK_PADDED }, .f32);
    ...
    var loop = k.openFor(0, TILES_PER_SPLIT, 1, .{ m_init, l_init, acc_init });
    {
        ...
        const V_lora = k.loadOpts(k.broadcastTo(v_base_ptrs, &.{ TILE_SIZE, VALUE_RANK_PADDED }).addPtr(k.broadcastTo(v_lora_dim_offsets, &.{ TILE_SIZE, VALUE_RANK_PADDED })), .{
            .mask = k.mask2d(valid_t, value_mask, TILE_SIZE, VALUE_RANK_PADDED),
            .other = k.zeros(&.{ TILE_SIZE, VALUE_RANK_PADDED }, config.kv_dtype),
            .cache_modifier = if (config.all_decode) .cg else .none,
        });
    ...
    acc = acc.mul(k.broadcastTo(one_over_l, &.{ BLOCK_M, VALUE_RANK_PADDED }));
    acc = k.where(
        has_value.expandDims(1),
        acc,
        k.zeros(&.{ BLOCK_M, VALUE_RANK_PADDED }, .f32),
    );
    ...
    if (three_d) {
        ...
        k.storeOpts(
            ...
            acc,
            .{ .mask = q_mask.bitAnd(value_mask.expandDims(0)) },
        );
...
fn runReduce3DPartials(b: *tri.Builder, cfg: Reduce3DPartialsCfg) tri.FinishError!void {
    ...
    const VALUE_RANK_PADDED: i64 = @intCast(std.math.ceilPowerOfTwoAssert(usize, @intCast(cfg.value_rank)));
    const offs_s = b.arange(0, splits_per_group, .i32);
    const offs_d = b.arange(0, VALUE_RANK_PADDED, .i32);
    const value_mask = offs_d.lt(@as(i32, @intCast(cfg.value_rank)));
    ...
    const input_output_offset = b.broadcastTo(
        ...
        &.{ splits_per_group, VALUE_RANK_PADDED },
    ).add(b.broadcastTo(offs_d.expandDims(0).to(.i64), &.{ splits_per_group, VALUE_RANK_PADDED }));
    const input_output = b.loadOpts(
        a.input_ptr.addPtr(input_output_offset),
        .{
            .mask = b.broadcastTo(value_mask.expandDims(0), &.{ splits_per_group, VALUE_RANK_PADDED }),
            .other = b.zeros(&.{ splits_per_group, VALUE_RANK_PADDED }, .f32),
        },
    );
    ...
    const output_offset = query_idx.to(.i64).mul(output_token_stride)
        ...
        .add(offs_d.to(.i64));
    b.storeOpts(a.partial_output_ptr.addPtr(output_offset), output, .{ .mask = value_mask });
...
fn runReduce3D(b: *tri.Builder, cfg: Reduce3DCfg) tri.FinishError!void {
    ...
    const partial_out_offset = b.broadcastTo(
        ...
        &.{ cfg.num_splits, VALUE_RANK_PADDED },
    ).add(b.broadcastTo(offs_d.expandDims(0).to(.i64), &.{ cfg.num_splits, VALUE_RANK_PADDED }));
    const partial_output = b.loadOpts(
        a.partial_output_ptr.addPtr(partial_out_offset),
        .{
            .mask = b.broadcastTo(value_mask.expandDims(0), &.{ cfg.num_splits, VALUE_RANK_PADDED }),
            .other = b.zeros(&.{ cfg.num_splits, VALUE_RANK_PADDED }, .f32),
        },
    );
