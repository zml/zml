    ...
    return struct {
        ...
        pub fn jsonParseFromValue(allocator: std.mem.Allocator, source: std.json.Value, options: std.json.ParseOptions) ParseFromValueError!T {
            inline for (comptime std.meta.fieldNames(T), comptime std.meta.fieldTypes(T)) |field_name, Field| {
                switch (Field) {
                    bool => if (source == .bool) return @unionInit(T, field_name, source.bool),
                    []const u8 => switch (source) {
                        .string, .number_string => |v| {
                            const dupe = try allocator.dupe(u8, v);
                            return @unionInit(T, field_name, dupe);
                        },
                        else => {},
                    },
                    else => switch (@typeInfo(Field)) {
                        .int => if (source == .integer) return @unionInit(T, field_name, @intCast(source.integer)),
                        .float => if (source == .float) return @unionInit(T, field_name, @floatCast(source.float)),
                        .@"struct" => if (source == .object) return @unionInit(T, field_name, try std.json.innerParseFromValue(Field, allocator, source, options)),
                        inline else => switch (source) {
                            .number_string, .array => return @unionInit(T, field_name, try std.json.innerParseFromValue(Field, allocator, source, options)),
                            else => {},
                        },
                    },
                }
...
pub fn TaggedUnionHelpers(comptime T: type, comptime tag_name: [:0]const u8) type {
    ...
    return struct {
        ...
        pub fn jsonParseFromValue(allocator: std.mem.Allocator, source: std.json.Value, options: std.json.ParseOptions) ParseFromValueError!T {
            ...
            inline for (comptime std.meta.fieldNames(T), comptime std.meta.fieldTypes(T)) |field_name, Field| {
                if (std.mem.eql(u8, field_name, tag.string)) {
                    ...
                    return @unionInit(T, field_name, inner);
                }
            }
        ...
        pub fn jsonStringify(self: T, jw: anytype) !void {
            ...
            switch (self) {
                inline else => |v, tag| {
                    // emit a field containing the enum tag.
...
test "strings in union can be freed" {
    ...
    try std.testing.expect(parsed.answer1 == .string);
    try std.testing.expectEqualStrings("I'm 29", parsed.answer1.string);
    std.testing.allocator.free(parsed.answer1.string);

    try std.testing.expect(parsed.answer2 == .string);
    try std.testing.expectEqualStrings("1e29", parsed.answer2.string);
    std.testing.allocator.free(parsed.answer2.string);
}
