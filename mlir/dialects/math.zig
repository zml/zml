//! Layer A builders for the `math` dialect.
//!
//! Intentionally omitted: `fastmath` on every float unary/binary/ternary,
//! `sincos`, `fpowi`, and the classification ops. It's a dialect-EnumAttr so a
//! CAPI shim would be needed to round-trip; Triton's TTIR lowering never sets
//! it (per-op precision modes come from `tt.precise_sqrt` / `tt.dot`'s
//! `InputPrecision`, not from `math.*` fastmath flags). Add on demand if a
//! kernel consumer needs it.
