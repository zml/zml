//! Layer A builders for the upstream `vector` dialect.
//!
//! The dialect is registered via `mlirGetDialectHandle__vector__` shipped by
//! `@llvm-project//mlir:CAPIVector`.
//!
//! Dialect-internal enum attributes (`CombiningKind`, `IteratorType`,
//! `PrintPunctuation`) are not exposed through the upstream public CAPI. We
//! build them by parsing their textual form (`#vector.kind<add>`,
//! `#vector.iterator_type<parallel>`, …); this keeps the wrapper free of a
//! C++ shim at the cost of a tiny per-op string parse during construction.
//! If perf becomes a concern we can switch to a CAPI shim later.
//!
//! Intentionally omitted (add on demand): `mask` (region-bearing op-mask
//! wrapper — rarely emitted directly), `scan` (lowering-only),
//! `outerproduct` (callers can synthesise from contract + broadcast),
//! `print` (host-only debug op), scalable-extract/insert variants.
