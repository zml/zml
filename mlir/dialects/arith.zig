//! Layer A builders for the `arith` dialect.
//!
//! Intentionally omitted (TTIR never emits these — add on demand if a kernel
//! needs them, but note they are dialect-EnumAttrs that won't round-trip via
//! plain `integerAttribute` and require a CAPI shim):
//!   * `fastmath` on addf/subf/mulf/divf/remf/negf/flush_denormals/
//!     maximumf/maxnumf/minimumf/minnumf/cmpf/extf/truncf/convertf/
//!     scaling_truncf/sitofp/uitofp/fptosi/fptoui
//!   * `overflowFlags` (nsw/nuw) on addi/subi/muli/shli/trunci
//!   * `isExact` on divsi/divui/shrsi/shrui (UnitAttr — no shim needed)
//!   * `roundingmode` on addf/subf/mulf/divf/truncf (defaults to RTE)
//!   * `nonNeg` on extui/uitofp/index_castui (UnitAttr — no shim needed)
//!
//! Triton's own TTIR lowering never sets any of these; they default to the
//! `none`/unset cases and canonical IR prints without them.
