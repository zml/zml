...
// =============================================================================
// draft/noise.zig — mt19937 + Box-Muller
//
// Draw order matches the official PyTorch path: video (C,T,H,W), then audio.
// =============================================================================

/// mt19937, 16-wide Box-Muller. Draw order: target `(C,T,H,W)`, then audio `(2*T,C)`.
const n = 624;
const m = 397;
...
pub fn drawVideo(
    ...
    return patchifyNchw(allocator, nchw, channels, latent_t, latent_h, latent_w, patch);
}
...
pub fn drawAudio(
    ...
    const out = try allocator.alloc(f32, @as(usize, 2) * audio_t * channels);
    randn(gen, out);
    return out;
}
