...
def create_causal_mask(
    ...
    _ = config, past_key_values, position_ids
    bsz, tgt_len = inputs_embeds.shape[:2]
    causal = torch.full((tgt_len, tgt_len), torch.finfo(inputs_embeds.dtype).min, device=inputs_embeds.device)
    causal = torch.triu(causal, diagonal=1).unsqueeze(0).unsqueeze(0).expand(bsz, 1, tgt_len, tgt_len)
    if attention_mask is None:
        return causal
    am = (1.0 - attention_mask[:, None, None, :].to(inputs_embeds.dtype)) * torch.finfo(inputs_embeds.dtype).min
    return causal + am
...
class Qwen3_5GatedDeltaNet(nn.Module):
    def __init__(self, config: Qwen3_5Config, layer_idx: int):
        ...
        # Keep only the default PyTorch implementations in this standalone test file.
...
class Qwen3_5TextModel(nn.Module):
    ...
    def _update_linear_attn_mask(self, attention_mask, past_key_values):
        ...
        if (past_key_values is not None and hasattr(past_key_values, "get_seq_length") and past_key_values.get_seq_length() > 0) or (
...
class Qwen3_5Model(nn.Module):
    ...
    def forward(
        ...
        if isinstance(outputs, Qwen3_5ModelOutputWithPast):
            return Qwen3_5ModelOutputWithPast(
