...
pub fn main(init: std.process.Init) !void {
    ...
    const number_video_pad_tokens: u32 = @intCast(
        @divExact(
            vision_inputs.video_grid_thw[0] * vision_inputs.video_grid_thw[1] * vision_inputs.video_grid_thw[2],
            merge_size * merge_size,
        ),
    );
    const prompt_tokens, const prompt_shape = try applyVideoChatTemplate(
        allocator,
        tokenizer,
        args.prompt,
        number_video_pad_tokens,
    );
...
fn applyVideoChatTemplate(
    ...
    var tokens: std.ArrayList(u32) = try .initCapacity(allocator, prompt.len + @as(usize, @intCast(number_video_pad_tokens)) + 16);
    try tokens.appendSlice(allocator, &.{ im_start_id, user, newline });
    try tokens.append(allocator, vision_start_id);
    for (0..number_video_pad_tokens) |_| {
        try tokens.append(allocator, video_pad_id);
    }
    ...
    const prompt_shape: [3]i64 = .{
        4,
        @intCast(number_video_pad_tokens),
