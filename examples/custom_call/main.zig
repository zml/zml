...
pub const AddOp = struct {
    ...
    pub fn init(
        ...
        const a = try zml.Buffer.fromWithOptions(platform, host_buffer, .{ .memory = .host_pinned });
        const b = try zml.Buffer.fromWithOptions(platform, host_buffer, .{ .memory = .host_pinned });
        const result = try zml.Buffer.fromWithOptions(platform, host_buffer, .{ .memory = .host_pinned });
