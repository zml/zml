...
const gpt_oss = @import("gpt_oss_flashinfer.zig");
...
pub fn main(init: std.process.Init) !void {
    ...
    const compiled_model_result = try compileModel(allocator, io, platform, gpt_model, gpt_parameters);
    ...
    var gpt_buffers = try GptOss.load(&gpt_model, allocator, io, platform, &store, &progress);
    ...
    log.info("Static SM90 MXFP4 preprocessing completed", .{});
...
fn parseConfig(allocator: std.mem.Allocator, io: std.Io, dir: std.Io.Dir) !std.json.Parsed(GptOss.Config) {
    ...
    defer log.info("Loaded model config [{D}]", .{stdx.fmt.fmtDuration(now.untilNow(io, .awake))});
...
const GptOssParameters = struct {
    ...
    metadata: flashinfer_moe.Metadata,
};
