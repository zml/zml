...
pub fn asyncMain() !void {
    ...
    const a_shape = zml.Shape.init(.{ 1, size }, dtype).withTags(.{ .m, .k }).withSharding(.{.k});
    ...
    const result_2_to_device = try result2.copyToMemory(platform, .device);
    var result3: zml.Buffer = executable.call(.{ result_2_to_device, buffer_b });
