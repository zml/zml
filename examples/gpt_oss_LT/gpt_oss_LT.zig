...
const MoE = struct {
    ...
    pub fn forward(self: MoE, input: Tensor, tokens_mask: ?Tensor, host_buffer_input: Tensor, device_buffer_input: Tensor) struct { Tensor, Tensor, Tensor } {
        ...
        // const weights_gate_up = self.experts.gate_up_proj.dequantize(dt);

        const weights_down = self.experts.down_proj.dequantize(dt).merge(.{ .expert = .{ .expert, .out } });
        // const weights_down = self.experts.down_proj.dequantize(dt);

        var host_buffer = host_buffer_input;
        var device_buffer = device_buffer_input;
        ...
        if (tokens_mask) |mask| {
            ...
            for (0..self.moe_opts.experts_per_token) |i| {
                ...
                log.info("weight shape: {f}", .{weights_gate_up.shape()});
                log.info("input shape: {f}", .{input_sorted.shape()});
                log.info("counts_tokens_per_exp shape: {f}", .{counts_tokens_per_exp.shape()});

                host_buffer, device_buffer, var out_gate_up = cublas_lt.gemmGroupedBatchedLT(
                    weights_gate_up,
                    input_sorted,
                ...
                host_buffer, device_buffer, var moe_out = cublas_lt.gemmGroupedBatchedLT(
                    weights_down,
                    out,
                    counts_tokens_per_exp,
                    host_buffer,
