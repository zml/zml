...
pub const Context = struct {
    ...
    pub fn init(
        ...
        const seed: u128 = @intCast((try std.Io.Clock.now(.real, io)).toNanoseconds());
        return .{
            .allocator = allocator,
            .io = io,
            .platform = platform,
    ...
    pub fn deinit(self: *Context) void {
        ...
        zml.attention.Metadata.deinitBuffer(&self.attention_metadata_buffers);
    ...
    pub fn start(self: *Context, prompt: []const u8) !void {
        ...
        while (self.remainingTokens() > 0) {
            ...
            try stdout.interface.print("\x1b[36mprefill\x1b[0m \x1b[2m{D} \xc2\xb7 {:.1}tok/s\x1b[0m \x1b[2m\xe2\x94\x82\x1b[0m \x1b[36mdecode\x1b[0m \x1b[2m{D} \xc2\xb7 {:.1}tok/s\x1b[0m\n\n\n", .{
                prefill_duration,
                prefill_duration.div(prefill_tokens).hzFloat(),
                decode_duration,
                decode_duration.div(decode_tokens).hzFloat(),
            });
            try stdout.interface.flush();
