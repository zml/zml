...
pub const Context = struct {
    ...
    fn runDecode(self: *Context, writer: *std.Io.Writer) !void {
        ...
        const think_start = self.tokenizer.findTokenId("<think>") orelse unreachable;
        const think_end = self.tokenizer.findTokenId("</think>") orelse unreachable;
...
fn tokenizePrompt(allocator: std.mem.Allocator, tokenizer: zml.tokenizer.Tokenizer, config: model.Config, prompt: []const u8) ![]u32 {
    ...
    const im_start = tokenizer.findTokenId("<|im_start|>") orelse return error.NoSuchToken;
    const im_end = tokenizer.findTokenId("<|im_end|>") orelse return error.NoSuchToken;
    const user = tokenizer.findTokenId("user") orelse return error.NoSuchToken;
    const assistant = tokenizer.findTokenId("assistant") orelse return error.NoSuchToken;
    const newline = (try encoder.feed("\n")).token_ids[0];
    ...
    const out = try encoder.encodeAlloc(allocator, prompt);
    defer out.deinit(allocator);
    try tokens.appendSlice(allocator, out.items);
