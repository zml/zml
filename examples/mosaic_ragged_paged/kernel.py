...
# Copyright 2026 ZML
#
# Vendored from JAX and modified by ZML. Slim cut of
# `platforms/tpu/kernel.py`: keeps only the bits that
# `ragged_paged_attention` actually pulls in (no `ref_*`, no
# `dynamic_validate_inputs`, no tuned-block-sizes auto-pick).
#
...
def main() -> None:
  ...
  head_dim = int(sys.argv[1]) if len(sys.argv) > 1 else HEAD_DIM
  kv_dtype_name = sys.argv[2] if len(sys.argv) > 2 else "bf16"
  _DTYPE_MAP = {
      "i8": jnp.int8, "i16": jnp.int16, "i32": jnp.int32,
      "f16": jnp.float16, "bf16": jnp.bfloat16, "f32": jnp.float32,
      "f8e4m3fn": jnp.float8_e4m3fn, "f8e5m2": jnp.float8_e5m2,
  }
  ...
  q = jax.ShapeDtypeStruct((NUM_Q_TOKENS, NUM_Q_HEADS, head_dim), jnp.bfloat16)
