...
const Args = struct {
    ...
    pub const help =
        ...
        \\  --verbose                  Print each selected prompt and generated answer
        \\
    ;
};
...
pub fn main(init: std.process.Init) !void {
    ...
    for (loaded_samples.samples, 0..) |sample, i| {
        ...
        const sample_steps = dflash_result.committed_lengths.items.len;
        const sample_committed = sumU32(dflash_result.committed_lengths.items);
        const sample_accepted = sumU32(dflash_result.acceptance_lengths.items);
        try writer.print("[{d}/{d}] dataset={s}/{s} id={s} prompt_tokens={d}\n", .{
            i + 1,
            loaded_samples.samples.len,
            @tagName(sample.source_dataset),
            sample.source_split,
            sample.id,
        ...
        try writer.print("  DFlash: {d} tokens, TPOT={d:.2}ms, TPS={d:.1}, tau={d:.2}, accepted_tau={d:.2}, steps={d}, eos={}\n", .{
            ...
            averageU64(sample_committed, sample_steps),
        ...
        if (args.verbose) {
            ...
            defer allocator.free(dflash_text_slice);
            try printVerboseDrySample(writer, sample.prompt, dflash_text_slice);
        }
        try writer.flush();

        summary.add(dflash_result);
    }

    try printDryRunReport(writer, summary);
    try writer.flush();
}

const DryRunSummary = struct {
    sample_count: usize = 0,
    generated_tokens: usize = 0,
    elapsed_ns: i128 = 0,
    stopped_on_eos_count: usize = 0,
    step_count: usize = 0,
    ...
    fn add(self: *DryRunSummary, result: MethodRunResult) void {
        self.sample_count += 1;
        self.generated_tokens += result.generated.items.len;
        self.elapsed_ns += result.decode_elapsed.nanoseconds;
        if (result.stopped_on_eos) self.stopped_on_eos_count += 1;
        self.step_count += result.committed_lengths.items.len;
        self.committed_total += sumU32(result.committed_lengths.items);
        self.accepted_total += sumU32(result.acceptance_lengths.items);
...
fn printDryRunReport(writer: *std.Io.Writer, summary: DryRunSummary) !void {
    try writer.writeAll(
        \\============================================================
        \\DRY RUN RESULTS
        \\============================================================
    ...
    try writer.print("DFlash TPOT:    {d:.2} ms ({d:.1} TPS)\n", .{
        tpotMs(.{ .nanoseconds = summary.elapsed_ns }, summary.generated_tokens),
        tokensPerSecond(.{ .nanoseconds = summary.elapsed_ns }, summary.generated_tokens),
    });
    ...
    for (summary.histogram, 0..) |count, valid_tokens| {
        const rate = if (histogram_total == 0) 0 else @as(f64, @floatFromInt(count)) / @as(f64, @floatFromInt(histogram_total));
...
fn averageU64(total: u64, count: usize) f64 {
    return if (count == 0) 0 else @as(f64, @floatFromInt(total)) / @as(f64, @floatFromInt(count));
}

fn barLen(rate: f64, max_width: usize) usize {
    if (rate <= 0) return 0;
    const scaled = rate * @as(f64, @floatFromInt(max_width));
    return @max(1, @as(usize, @intFromFloat(@round(scaled))));
}

fn tpotMs(duration: stats.Duration, tokens: usize) f64 {
    if (tokens == 0) return 0;
    return (@as(f64, @floatFromInt(duration.nanoseconds)) / std.time.ns_per_ms) / @as(f64, @floatFromInt(tokens));
}

fn tokensPerSecond(duration: stats.Duration, tokens: usize) f64 {
    if (tokens == 0 or duration.nanoseconds == 0) return 0;
    return @as(f64, @floatFromInt(tokens)) / (@as(f64, @floatFromInt(duration.nanoseconds)) / std.time.ns_per_s);
}

fn printVerboseDrySample(
