...
DEFAULT_MODEL = "/var/models/meta-llama/Llama-3.1-8B-Instruct/"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Benchmark a running SGLang OpenAI-compatible server on JSONL "
            "dataset samples."
        )
    )
    parser.add_argument(
        ...
        help=(
            "JSONL file(s) under examples/dflash_benchmark/data, or explicit paths. "
            "Default: math500.jsonl stories.jsonl"
        ),
    )
    parser.add_argument(
