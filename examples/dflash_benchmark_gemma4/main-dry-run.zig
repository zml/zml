...
const gemma4_inference = @import("gemma4/gemma4_inference.zig");
const chat_template = @import("chat_template.zig");
const gemma4 = @import("gemma4/gemma4_model.zig");
const stats = @import("stats.zig");

pub const std_options: std.Options = .{
    .log_level = .info,
};
const log = std.log.scoped(.dflash_benchmark_gemma4);
...
const Args = struct {
    ...
    pub const help =
        ...
        \\  --target-model=<path>      Path to the target Gemma 4 model repository
...
const Project = struct {
    ...
    parsed_target_config: gemma4.ParsedConfig,
    ...
    pub fn deinit(self: *Project) void {
        ...
        self.chat_template.deinit(allocator);
...
fn initProject(init: std.process.Init, args: Args) !*Project {
    ...
    project.parsed_target_config = try gemma4.parseConfig(allocator, project.io, project.target_repo);
    errdefer project.parsed_target_config.deinit();
    try validateProjectConfig(project);
    ...
    project.tokenizer = try chat_template.loadTokenizer(allocator, project.io, project.target_repo);
    errdefer project.tokenizer.deinit();

    project.chat_template = try .load(allocator, project.io, project.target_repo);
    errdefer project.chat_template.deinit(allocator);
...
fn tokenizePrompt(project: *Project, prompt_text: []const u8, max_new_tokens: u32) !TokenizedPrompt {
    const token_ids = try project.chat_template.tokenizePrompt(project.allocator, &project.tokenizer, prompt_text);
...
const ModelsAndCaches = struct {
    ...
    target_kv_cache: gemma4.KvCache,
    draft_kv_cache: dflash.KvCache,
    rng: zml.Tensor.Rng,
    sampling: gemma4.SamplingConfig,
    target_layers: gemma4_inference.TargetLayers,
    block_size: u32,

    pub fn deinit(self: ModelsAndCaches, allocator: std.mem.Allocator) void {
        self.target_kv_cache.deinit(allocator);
        self.target_model.deinit(allocator);
        self.draft_model.deinit(allocator);
    }
};

fn validateProjectConfig(project: *const Project) !void {
    const config = project.parsed_config.value;
    if (config.block_size < 2) return error.InvalidDFlashBlockSize;
    if (config.dflash_config.mask_token_id == null) return error.MissingDFlashMaskToken;
    if (config.dflash_config.target_layer_ids.len == 0) return error.MissingDFlashTargetLayers;

    const target_config = project.parsed_target_config.value;
    for (config.dflash_config.target_layer_ids) |layer_id| {
        if (layer_id >= target_config.num_hidden_layers) return error.InvalidDFlashTargetLayer;
    }
}
...
fn initModelsAndCaches(project: *Project, args: Args, selected_max_prompt_tokens: u32) !ModelsAndCaches {
    ...
    const target_model = try gemma4.Model.init(project.allocator, project.target_store.view(), project.parsed_target_config.value);
    ...
    return .{
        ...
        .target_kv_cache = try target_model.initKvCache(project.allocator, target_model.config, cache_seq_len),
...
fn validateModelCompatibility(draft_model: dflash.Model, target_model: gemma4.Model) !void {
    const expected_target_width: i64 = @intCast(draft_model.target_layer_ids.len * @as(usize, @intCast(target_model.config.hidden_size)));
    const actual_target_width = draft_model.fc.weight.dim(.d);
    if (actual_target_width != expected_target_width) {
        log.err("DFlash fc.weight input width {} does not match {} target layers * Gemma hidden size {}", .{ actual_target_width, draft_model.target_layer_ids.len, target_model.config.hidden_size });
        return error.InvalidDFlashTargetProjection;
    }
}
...
fn compileShared(allocator: std.mem.Allocator, project: *Project, models: ModelsAndCaches) !CompiledExes {
    ...
    const target_prefill_exe = try gemma4_inference.compileTargetPrefill(
        allocator,
        project.io,
        project.platform,
        models.target_model,
    ...
    const context_precompute_target_hidden_tensor = gemma4_inference.targetHiddenTensor(models.target_model, project.parsed_target_config.value, models.target_layers, default_prefill_seq_len);
    ...
    const prefill_target_hidden_tensor = gemma4_inference.targetHiddenTensor(models.target_model, project.parsed_target_config.value, models.target_layers, default_prefill_seq_len);
    ...
    const draft_target_hidden_tensor = gemma4_inference.targetHiddenTensor(models.target_model, project.parsed_target_config.value, models.target_layers, models.block_size);
    ...
    const target_verify_exe = try gemma4_inference.compileTargetVerify(
        allocator,
        project.io,
        project.platform,
        models.target_model,
...
const LoadedBuffers = struct {
    ...
    target_buffers: gemma4.Buffers,

    pub fn deinit(self: *LoadedBuffers, allocator: std.mem.Allocator) void {
        gemma4.Model.unloadBuffers(&self.target_buffers, allocator);
...
fn runPrefill(
    ...
    target_kv_cache_buffers: *gemma4.KvCache.Buffer,
    ...
    const target_hidden_buffer, var target_token_buffer, var prefilled_target_kv_cache_buffers, var updated_rng_buffer = prefill_results.get(struct {
        zml.Buffer,
        zml.Buffer,
        gemma4.KvCache.Buffer,
        zml.Tensor.Rng.Buffer,
    });
    defer target_token_buffer.deinit();
    gemma4.KvCache.replaceBuffers(target_kv_cache_buffers, &prefilled_target_kv_cache_buffers);
...
const RuntimeState = struct {
    ...
    pub fn deinit(self: *RuntimeState) void {
        gemma4.KvCache.deinitBuffer(&self.target_kv_cache_buffers);
...
fn initInputAndState(
    ...
    var target_kv_cache_buffers = try models.target_kv_cache.initZeroBuffer(project.allocator, project.io, project.platform, .{
        .sliding = project.shardings.model,
        .full = project.shardings.model,
    });
    errdefer gemma4.KvCache.deinitBuffer(&target_kv_cache_buffers);
...
fn runBaseline(
    ...
    var stopped_on_eos = gemma4.isEosToken(&project.parsed_target_config.value, current_token);
    ...
    while (absolute_pos < prompt.max_seq_len and !stopped_on_eos) : (absolute_pos += 1) {
        ...
        gemma4.KvCache.replaceBuffers(&state.target_kv_cache_buffers, &updated_target_kv_cache_buffers);
        ...
        if (gemma4.isEosToken(&project.parsed_target_config.value, current_token)) {
            stopped_on_eos = true;
        } else {
...
fn runDFlash(
    ...
    while (start < prompt.max_seq_len and !stopped_on_eos) {
        ...
        for (block_tokens[0..@as(usize, @intCast(committed_tokens))]) |token| {
            if (gemma4.isEosToken(&project.parsed_target_config.value, token)) {
                stopped_on_eos = true;
                break;
            }
        ...
        if (!stopped_on_eos and start < prompt.max_seq_len) {
            if (gemma4.isEosToken(&project.parsed_target_config.value, correction_token)) {
                stopped_on_eos = true;
            } else {
...
fn runVerification(
    ...
    gemma4.KvCache.replaceBuffers(target_kv_cache_buffers, &verified_target_kv_cache_buffers);
    replaceRngBuffer(rng_buffer, &updated_rng_buffer);
...
fn targetDecode(
    ...
    rng: zml.Tensor.Rng,
    sampling: gemma4.SamplingConfig,
) struct { zml.Tensor, gemma4.KvCache, zml.Tensor.Rng } {
