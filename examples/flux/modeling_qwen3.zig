...
pub const Qwen3ForCausalLM = struct {
    ...
    pub fn pipelineRun(allocator: std.mem.Allocator, io: std.Io, repo_dir: std.Io.Dir, platform: *const zml.Platform, progress: ?*std.Progress.Node, parallelism_level: usize, tokens: Qwen2TokenizerFast.TokenizeOutput) !EmbedingOutput {
        var qwen3_model_ctx = try Qwen3ForCausalLM.loadFromFile(allocator, io, platform, repo_dir, parallelism_level, progress);
        ...
        log.info("Computing RoPE with seq_len {d} and head_dim {d}...", .{ seq_len, head_dim });
        var rope = try computeRoPE(allocator, io, platform, seq_len, head_dim, qwen3_model_ctx.config.rope_theta);
        defer rope.deinit();
        log.info("RoPE computed successfully.", .{});
