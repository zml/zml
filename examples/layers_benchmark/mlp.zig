...
pub fn main() !void {
    ...
    var mlp_tensors: Mlp = try .init(store.view());
    defer mlp_tensors.deinit();

    var mlp_buffers = try mlp_tensors.loadBuffers(allocator, io, store.view(), platform);
    defer Mlp.unloadBuffers(&mlp_buffers);
    ...
    const batch_sizes = [_]u32{ 448, 464, 480, 496, 512 };
    ...
    for (batch_sizes, 0..) |batch_size, j| {
        ...
        var exe = try platform.compileModel(allocator, io, Mlp.forward, mlp_tensors, .{input_tensor});
        defer exe.deinit();
        ...
        args.set(.{ mlp_buffers, input_buffer });
