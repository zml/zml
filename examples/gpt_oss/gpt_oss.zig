...
pub const SelfAttn = struct {
    ...
    pub fn forward(
        ...
        log.info("pos_index shape: {f}", .{pos_index.shape()});
        log.info("token_index shape: {f}", .{token_index.shape()});
...
pub fn moePerTokenRouting(Expert: type, experts: Expert, input: zml.Tensor, gating: zml.Tensor, opts: MoeOpts) zml.Tensor {
    ...
    var per_token_outputs_buf: [32]Tensor = undefined;
    const per_token_outputs = per_token_outputs_buf[0..num_tokens];
