...
pub fn tokenizePrompt(allocator: std.mem.Allocator, tokenizer: zml.tokenizer.Tokenizer, config: GptOss.Config, prompt: []const u8, skip_llama3_encoding: bool) ![]u32 {
    ...
    std.log.warn("Detokenizes too: {s}", .{try decoder.decode(tokens.items)});
...
pub fn asyncMain() !void {
    ...
    const config = config: {
        ...
        defer reader.deinit();
        var config = try std.json.parseFromTokenSourceLeaky(GptOss.Config, allocator, &reader, .{ .ignore_unknown_fields = true });
    ...
    const options: GptOss.Options = .{
        .max_seq_len = cli.args.@"seq-len" orelse 256,
        .max_prompt_len = 64,
        .tokens_per_expert_ratio = cli.args.@"expert-budget" orelse 2.0,
        .sampling_strategy = .{
            .topk = 1,
            .temperature = 1.0,
        },
    ...
    const model_instance = try GptOss.init(compiler_arena.allocator(), store, config, options);
    ...
    try generateText(config, options, mixtral_module_prefill, mixtral_module, kv_cache, tokenizer, allocator, seed, prompt[0..], skip_llama3_encoding, &output.interface);
