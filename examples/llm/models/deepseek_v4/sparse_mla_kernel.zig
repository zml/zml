...
fn run(b: *tri.Builder, cfg: Cfg) tri.FinishError!void {
    ...
    if (cfg.use_attn_sink) {
        const sink_logits = b.loadOpts(a.sink_ptr.addPtr(cur_head.to(.i64)), .{
            .mask = mask_h,
            .other = b.zeros(&.{cfg.BLOCK_H}, cfg.sink_dtype),
        }).to(.f32).mul(b.full(&.{cfg.BLOCK_H}, cfg.LOG2E, .f32));
        const sink_score = b.where(
            mask_h,
            sink_logits,
            b.full(&.{cfg.BLOCK_H}, -std.math.inf(f32), .f32),
        );
        const n_e_max = sink_score.maximum(e_max);
        const re_scale = b.exp2(e_max.sub(n_e_max));
        const sink_p = b.exp2(sink_score.sub(n_e_max));

        acc = re_scale.expandDims(1).mul(acc);
        e_sum = e_sum.mul(re_scale).add(sink_p);
        e_max = n_e_max;
    }
...
pub fn args() std.meta.ArgsTuple(@TypeOf(forward)) {
    ...
    return .{
        ...
        Tensor.init(.{NUM_HEADS_Q}, .bf16), // sink_ptr
