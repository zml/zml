...
pub const Shardings = struct {
    ...
    pub fn init(platform: *zml.Platform) !Shardings {
        // const experts_strategy = strategyOverShardableAxes(platform, .experts);
    ...
    // fn strategyOverShardableAxes(platform: *const zml.Platform, comptime logical_axis: anytype) zml.Sharding.Strategy {
    //     var strategy: zml.Sharding.Strategy = .init;
    //     for (platform.physical_mesh.shardableAxes()) |axis| {
    //         if (platform.physical_mesh.hasAxis(axis)) {
    //             strategy.addBinding(logical_axis, axis);
    //         }
    //     }
    //     return strategy;
    // }
