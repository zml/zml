...
# This file produces identical outputs to ./patched_config/modeling_step3p5.py
# for the shipped Step3p5 config because of these baked-in assumptions:
#   - config.rope_parameters is unused (we always take the "default" rope_type
#     path); rope_init_fn ignores rope_parameters for rope_type=="default".
#   - config has no partial_rotary_factors list; we set partial_rotary_factor=1.0,
#     the same fallback the original used when that list was absent.
#   - config.rope_theta is the per-layer list form (.copy()/index works).
#   - config.layer_types is non-empty and aligned with num_hidden_layers.
#   - config.use_moe_router_bias is True (only the router-bias path is kept).
#   - config.need_fp32_gate is True (we always do the fp32 gate matmul).
#   - config.use_head_wise_attn_gate is True (g_proj/gate path is unconditional).
#   - config has no use_rope_layers (RoPE is on for every layer).
#   - Inference only: dropout is 0 and training==False, so the missing
#     self.attention_dropout never matters.
#   - output_attentions / output_hidden_states are unused: the original also
#     never populated attn weights and only collected pre-layer hidden states
#     into a discarded field, so observable outputs match.
#   - Text-only: num_patches/patch_pixel_values/patch_newline_mask/pixel_values
#     were silently dropped by **kwargs in the original.
...
class Step3p5MoEMLP(nn.Module):
    ...
    def route(self, gating_output: torch.Tensor, topk: int,
              renormalize: bool):
    ...
    def forward(self, hidden_states):
        ...
        routing_weights, selected_experts = self.route(
