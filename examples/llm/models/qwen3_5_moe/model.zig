...
pub const LoadedModel = struct {
    ...
    pub fn loadBuffers(
        ...
        try self.packPerExpertMoeWeights(allocator, io, platform, &buffers, all_shardings);
        return buffers;
    }

    fn packPerExpertMoeWeights(
        self: *LoadedModel,
        allocator: std.mem.Allocator,
        io: std.Io,
        platform: *const zml.Platform,
        ...
        for (self.inner.text_model.layers, buffers.text_model.layers) |*layer_model, *layer_buffers| {
            ...
            if (pack_gate_up_exe == null) {
                pack_gate_up_exe = try platform.compileFn(
                    ...
                    .{ layer_model.moe.multiple_gate_proj, layer_model.moe.multiple_up_proj },
                    .{ .shardings = &shardings },
                );
                pack_gate_up_scale_exe = try platform.compileFn(
                    ...
                    io,
                    MoeRepacker.packGateUp,
                    .{ layer_model.moe.multiple_gate_proj_scale, layer_model.moe.multiple_up_proj_scale },
                    .{ .shardings = &shardings },
                );
                pack_down_exe = try platform.compileFn(
                    ...
                    .{layer_model.moe.multiple_down_proj},
                    .{ .shardings = &shardings },
                );
                pack_down_scale_exe = try platform.compileFn(
                    ...
                    io,
                    MoeRepacker.packExperts,
                    .{layer_model.moe.multiple_down_proj_scale},
                    .{ .shardings = &shardings },
                );
            }
            ...
            for (layer_buffers.moe.multiple_gate_proj, 0..) |buffer, i| {
                gate_up_args.setPartial(buffer, i);
            }
            for (layer_buffers.moe.multiple_up_proj, 0..) |buffer, i| {
                gate_up_args.setPartial(buffer, layer_buffers.moe.multiple_gate_proj.len + i);
            }
            ...
            for (layer_buffers.moe.multiple_gate_proj_scale, 0..) |buffer, i| {
                gate_up_scale_args.setPartial(buffer, i);
            }
            for (layer_buffers.moe.multiple_up_proj_scale, 0..) |buffer, i| {
                gate_up_scale_args.setPartial(buffer, layer_buffers.moe.multiple_gate_proj_scale.len + i);
            }
            ...
            for (layer_buffers.moe.multiple_down_proj, 0..) |buffer, i| {
            ...
            for (layer_buffers.moe.multiple_down_proj_scale, 0..) |buffer, i| {
            ...
            const empty_multiple_gate_proj = try allocator.alloc(zml.Tensor, 0);
            errdefer allocator.free(empty_multiple_gate_proj);
            const empty_multiple_gate_proj_scale = try allocator.alloc(zml.Tensor, 0);
            errdefer allocator.free(empty_multiple_gate_proj_scale);
            const empty_multiple_up_proj = try allocator.alloc(zml.Tensor, 0);
            errdefer allocator.free(empty_multiple_up_proj);
            const empty_multiple_up_proj_scale = try allocator.alloc(zml.Tensor, 0);
            errdefer allocator.free(empty_multiple_up_proj_scale);
            const empty_multiple_down_proj = try allocator.alloc(zml.Tensor, 0);
            errdefer allocator.free(empty_multiple_down_proj);
            const empty_multiple_down_proj_scale = try allocator.alloc(zml.Tensor, 0);
            errdefer allocator.free(empty_multiple_down_proj_scale);

            const empty_multiple_gate_proj_buffers = try allocator.alloc(zml.Buffer, 0);
            errdefer allocator.free(empty_multiple_gate_proj_buffers);
            const empty_multiple_gate_proj_scale_buffers = try allocator.alloc(zml.Buffer, 0);
            errdefer allocator.free(empty_multiple_gate_proj_scale_buffers);
            const empty_multiple_up_proj_buffers = try allocator.alloc(zml.Buffer, 0);
            errdefer allocator.free(empty_multiple_up_proj_buffers);
            const empty_multiple_up_proj_scale_buffers = try allocator.alloc(zml.Buffer, 0);
            errdefer allocator.free(empty_multiple_up_proj_scale_buffers);
            const empty_multiple_down_proj_buffers = try allocator.alloc(zml.Buffer, 0);
            errdefer allocator.free(empty_multiple_down_proj_buffers);
            const empty_multiple_down_proj_scale_buffers = try allocator.alloc(zml.Buffer, 0);
            errdefer allocator.free(empty_multiple_down_proj_scale_buffers);

            allocator.free(layer_model.moe.multiple_gate_proj);
            allocator.free(layer_model.moe.multiple_gate_proj_scale);
            allocator.free(layer_model.moe.multiple_up_proj);
            allocator.free(layer_model.moe.multiple_up_proj_scale);
            allocator.free(layer_model.moe.multiple_down_proj);
            allocator.free(layer_model.moe.multiple_down_proj_scale);

            for (layer_buffers.moe.multiple_gate_proj) |*buffer| buffer.deinit();
            allocator.free(layer_buffers.moe.multiple_gate_proj);
            for (layer_buffers.moe.multiple_gate_proj_scale) |*buffer| buffer.deinit();
            allocator.free(layer_buffers.moe.multiple_gate_proj_scale);
            for (layer_buffers.moe.multiple_up_proj) |*buffer| buffer.deinit();
            allocator.free(layer_buffers.moe.multiple_up_proj);
            for (layer_buffers.moe.multiple_up_proj_scale) |*buffer| buffer.deinit();
            allocator.free(layer_buffers.moe.multiple_up_proj_scale);
            for (layer_buffers.moe.multiple_down_proj) |*buffer| buffer.deinit();
            allocator.free(layer_buffers.moe.multiple_down_proj);
            for (layer_buffers.moe.multiple_down_proj_scale) |*buffer| buffer.deinit();
            allocator.free(layer_buffers.moe.multiple_down_proj_scale);
            ...
            layer_model.moe.multiple_gate_proj = empty_multiple_gate_proj;
            layer_model.moe.multiple_gate_proj_scale = empty_multiple_gate_proj_scale;
            layer_model.moe.multiple_up_proj = empty_multiple_up_proj;
            layer_model.moe.multiple_up_proj_scale = empty_multiple_up_proj_scale;
            layer_model.moe.multiple_down_proj = empty_multiple_down_proj;
            layer_model.moe.multiple_down_proj_scale = empty_multiple_down_proj_scale;
            ...
            layer_buffers.moe.multiple_gate_proj = empty_multiple_gate_proj_buffers;
            layer_buffers.moe.multiple_gate_proj_scale = empty_multiple_gate_proj_scale_buffers;
            layer_buffers.moe.multiple_up_proj = empty_multiple_up_proj_buffers;
            layer_buffers.moe.multiple_up_proj_scale = empty_multiple_up_proj_scale_buffers;
            layer_buffers.moe.multiple_down_proj = empty_multiple_down_proj_buffers;
            layer_buffers.moe.multiple_down_proj_scale = empty_multiple_down_proj_scale_buffers;
        }
    }
...
pub const Moe = struct {
    ...
    gate_up_proj: ?zml.Tensor,
    gate_up_proj_scale: ?zml.Tensor,
    multiple_gate_proj: []zml.Tensor,
    multiple_gate_proj_scale: []zml.Tensor,
    multiple_up_proj: []zml.Tensor,
    multiple_up_proj_scale: []zml.Tensor,
    down_proj: ?zml.Tensor,
    down_proj_scale: ?zml.Tensor,
    multiple_down_proj: []zml.Tensor,
    multiple_down_proj_scale: []zml.Tensor,
    router: Router,

    fn concatenateExperts(tensors: []const zml.Tensor, axis_: anytype) zml.Tensor {
        stdx.debug.assert(tensors.len > 0, "expected at least one tensor to concatenate", .{});
        if (tensors.len <= 32) return zml.Tensor.concatenate(tensors, axis_);

        var partials = stdx.BoundedArray(zml.Tensor, 32).init(0) catch unreachable;
        var start: usize = 0;
        while (start < tensors.len) {
            const end = @min(start + 32, tensors.len);
            partials.appendAssumeCapacity(concatenateExperts(tensors[start..end], axis_));
            start = end;
        }

        return concatenateExperts(partials.constSlice(), axis_);
    }
    ...
    fn repackExperts(tensors: []const zml.Tensor, axis_: anytype, tag: anytype) zml.Tensor {
        ...
        if (tensors.len <= 32) {
            var reshaped: [32]zml.Tensor = undefined;
            for (tensors, 0..) |tensor, i| {
                reshaped[i] = tensor.reshape(stacked_shape);
            }
            return zml.Tensor.concatenate(reshaped[0..tensors.len], axis_);
        }
        ...
        while (start < tensors.len) {
            const end = @min(start + 32, tensors.len);
            partials.appendAssumeCapacity(repackExperts(tensors[start..end], axis_, tag));
            start = end;
        }
    ...
    pub fn init(allocator: std.mem.Allocator, store: zml.io.TensorStore.View, config: Config) !Moe {
        ...
        if (num_experts > max_num_experts) return error.TooManyExperts;

        var multiple_gate_proj = try allocator.alloc(zml.Tensor, 0);
        errdefer allocator.free(multiple_gate_proj);
        var multiple_gate_proj_scale = try allocator.alloc(zml.Tensor, 0);
        errdefer allocator.free(multiple_gate_proj_scale);
        var multiple_up_proj = try allocator.alloc(zml.Tensor, 0);
        errdefer allocator.free(multiple_up_proj);
        var multiple_up_proj_scale = try allocator.alloc(zml.Tensor, 0);
        errdefer allocator.free(multiple_up_proj_scale);
        var multiple_down_proj = try allocator.alloc(zml.Tensor, 0);
        errdefer allocator.free(multiple_down_proj);
        var multiple_down_proj_scale = try allocator.alloc(zml.Tensor, 0);
        errdefer allocator.free(multiple_down_proj_scale);

        var layout: Layout = .packed_weights;

        if (gate_up_proj_tensor == null) {
            layout = .per_expert;
            const new_multiple_gate_proj = try initPerExpertTable(allocator, experts_store, num_experts, "gate_proj", "weight", .{ .dout, .d });
            allocator.free(multiple_gate_proj);
            multiple_gate_proj = new_multiple_gate_proj;

            const new_multiple_gate_proj_scale = try initPerExpertTable(allocator, experts_store, num_experts, "gate_proj", "weight_scale_inv", .{ .dout, .d });
            allocator.free(multiple_gate_proj_scale);
            multiple_gate_proj_scale = new_multiple_gate_proj_scale;

            const new_multiple_up_proj = try initPerExpertTable(allocator, experts_store, num_experts, "up_proj", "weight", .{ .dout, .d });
            allocator.free(multiple_up_proj);
            multiple_up_proj = new_multiple_up_proj;

            const new_multiple_up_proj_scale = try initPerExpertTable(allocator, experts_store, num_experts, "up_proj", "weight_scale_inv", .{ .dout, .d });
            allocator.free(multiple_up_proj_scale);
            multiple_up_proj_scale = new_multiple_up_proj_scale;

            const new_multiple_down_proj = try initPerExpertTable(allocator, experts_store, num_experts, "down_proj", "weight", .{ .d, .dout });
            allocator.free(multiple_down_proj);
            multiple_down_proj = new_multiple_down_proj;

            const new_multiple_down_proj_scale = try initPerExpertTable(allocator, experts_store, num_experts, "down_proj", "weight_scale_inv", .{ .d, .dout });
            allocator.free(multiple_down_proj_scale);
            multiple_down_proj_scale = new_multiple_down_proj_scale;
        ...
        return .{
            ...
            .multiple_gate_proj = multiple_gate_proj,
            .multiple_gate_proj_scale = multiple_gate_proj_scale,
            .multiple_up_proj = multiple_up_proj,
            .multiple_up_proj_scale = multiple_up_proj_scale,
            .down_proj = down_proj_tensor,
            .down_proj_scale = null,
            .multiple_down_proj = multiple_down_proj,
            .multiple_down_proj_scale = multiple_down_proj_scale,
            .router = Router.init(store.withPrefix("gate"), config.text_config.num_experts_per_tok.?),
        };
    }

    pub fn packPerExpertWeights(self: Moe) PackedWeights {
        stdx.debug.assert(self.layout == .per_expert, "expected per-expert MoE layout", .{});
        stdx.debug.assert(self.multiple_gate_proj.len > 0, "expected non-empty gate projection table", .{});
        stdx.debug.assert(self.multiple_gate_proj.len == self.multiple_gate_proj_scale.len, "gate projection and scale tables must match", .{});
        stdx.debug.assert(self.multiple_gate_proj.len == self.multiple_up_proj.len, "gate and up projection tables must match", .{});
        stdx.debug.assert(self.multiple_gate_proj.len == self.multiple_up_proj_scale.len, "gate and up scale tables must match", .{});
        stdx.debug.assert(self.multiple_gate_proj.len == self.multiple_down_proj.len, "gate and down projection tables must match", .{});
        stdx.debug.assert(self.multiple_gate_proj.len == self.multiple_down_proj_scale.len, "gate and down scale tables must match", .{});

        const gate_proj = repackExperts(self.multiple_gate_proj, 0, .expert);
        const gate_proj_scale = repackExperts(self.multiple_gate_proj_scale, 0, .expert);
        const up_proj = repackExperts(self.multiple_up_proj, 0, .expert);
        const up_proj_scale = repackExperts(self.multiple_up_proj_scale, 0, .expert);

        return .{
            .gate_up_proj = zml.Tensor.concatenate(&.{ gate_proj, up_proj }, .dout),
            .gate_up_proj_scale = zml.Tensor.concatenate(&.{ gate_proj_scale, up_proj_scale }, .dout),
            .down_proj = repackExperts(self.multiple_down_proj, 0, .expert),
            .down_proj_scale = repackExperts(self.multiple_down_proj_scale, 0, .expert),
        };
    }

    pub fn deinit(self: Moe, allocator: std.mem.Allocator) void {
        allocator.free(self.multiple_gate_proj);
        allocator.free(self.multiple_gate_proj_scale);
        allocator.free(self.multiple_up_proj);
        allocator.free(self.multiple_up_proj_scale);
        allocator.free(self.multiple_down_proj);
        allocator.free(self.multiple_down_proj_scale);
    }
    ...
    pub fn forward(self: Moe, x: zml.Tensor, moe_metadata: zml.moe.Metadata, moe_parameters: zml.moe.Parameters) zml.Tensor {
        return switch (self.layout) {
            .packed_weights => if (self.gate_up_proj_scale != null and self.down_proj_scale != null)
                self.forward_quantized(x, moe_metadata, moe_parameters)
            else
                self.forward_unquantized(x, moe_metadata, moe_parameters),
            .per_expert => self.forward_quantized(x, moe_metadata, moe_parameters),
        };
    }

    pub fn forward_unquantized(self: Moe, x: zml.Tensor, moe_metadata: zml.moe.Metadata, moe_parameters: zml.moe.Parameters) zml.Tensor {
    ...
    pub fn forward_quantized(self: Moe, x: zml.Tensor, moe_metadata: zml.moe.Metadata, moe_parameters: zml.moe.Parameters) zml.Tensor {
    ...
    pub fn unloadBuffers(self: *zml.Bufferized(Moe), allocator: std.mem.Allocator) void {
        ...
        for (self.multiple_gate_proj) |*tensor| tensor.deinit();
        allocator.free(self.multiple_gate_proj);
        for (self.multiple_gate_proj_scale) |*tensor| tensor.deinit();
        allocator.free(self.multiple_gate_proj_scale);
        for (self.multiple_up_proj) |*tensor| tensor.deinit();
        allocator.free(self.multiple_up_proj);
        for (self.multiple_up_proj_scale) |*tensor| tensor.deinit();
        allocator.free(self.multiple_up_proj_scale);
        if (self.down_proj) |*down_proj| down_proj.deinit();
        if (self.down_proj_scale) |*down_proj_scale| down_proj_scale.deinit();
        for (self.multiple_down_proj) |*tensor| tensor.deinit();
        allocator.free(self.multiple_down_proj);
        for (self.multiple_down_proj_scale) |*tensor| tensor.deinit();
        allocator.free(self.multiple_down_proj_scale);
        Router.unloadBuffers(&self.router);
    }
};

pub fn dotDequantizeFp8(
    lhs: zml.Tensor,
    rhs: zml.Tensor,
    rhs_scale_inv: zml.Tensor,
) zml.Tensor {
    stdx.debug.assert(lhs.rank() == 4, "expected a 4D blocked lhs tensor, got {f}", .{lhs.shape()});
    stdx.debug.assert(rhs.rank() == 4, "expected a 4D blocked rhs tensor, got {f}", .{rhs.shape()});
    stdx.debug.assert(rhs_scale_inv.rank() == 4, "expected a 4D blocked rhs scale tensor, got {f}", .{rhs_scale_inv.shape()});
    stdx.debug.assert(rhs.dtype() == .f8e4m3fn, "rhs must be an FP8 quantized tensor, got {}", .{rhs.dtype()});
    stdx.debug.assert(lhs.dim(.d_chunk) == 128 and lhs.dim(.token_chunk) == 128, "lhs must use 128x128 activation blocks, got {f}", .{lhs.shape()});
    stdx.debug.assert(rhs.dim(.dout_chunk) == 128 and rhs.dim(.d_chunk) == 128, "rhs must use 128x128 weight blocks, got {f}", .{rhs.shape()});
    stdx.debug.assert(rhs_scale_inv.dim(.dout_chunk) == 1 and rhs_scale_inv.dim(.d_chunk) == 1, "rhs scale must use 1x1 scale blocks, got {f}", .{rhs_scale_inv.shape()});
    stdx.debug.assert(lhs.dim(.d_block) == rhs.dim(.d_block), "lhs and rhs input block counts must match, got {f} and {f}", .{ lhs.shape(), rhs.shape() });
    stdx.debug.assert(rhs.dim(.dout_block) == rhs_scale_inv.dim(.dout_block) and rhs.dim(.d_block) == rhs_scale_inv.dim(.d_block), "rhs scale blocks must match rhs weight blocks, got {f} and {f}", .{ rhs.shape(), rhs_scale_inv.shape() });

    const rhs_dequantized = rhs.convert(.bf16).div(rhs_scale_inv.convert(.bf16));

    return lhs.dotGeneral(
        rhs_dequantized,
        &.{
            .{ @intCast(lhs.axis(.d_block)), @intCast(rhs_dequantized.axis(.d_block)) },
            .{ @intCast(lhs.axis(.d_chunk)), @intCast(rhs_dequantized.axis(.d_chunk)) },
        },
        &.{},
    );
}
...
fn forwardLinearDequantizeFp8(x: zml.Tensor, linear: zml.nn.Linear, scale_inv: ?zml.Tensor) zml.Tensor {
    if (scale_inv == null) return linear.forward(x);
    ...
    stdx.debug.assert(@mod(x.dim(-1), 128) == 0, "input dim must be divisible by 128, got {d}", .{x.dim(-1)});
    ...
    const token_remainder = @mod(num_tokens, 128);
    const padding_tokens: i64 = if (token_remainder == 0) 0 else 128 - token_remainder;

    var flat_x = x.reshape(.{ .token = num_tokens, .d = x.dim(-1) });
    if (padding_tokens > 0) {
        const padding = zml.Tensor.constant(x.dtype().zero())
            .broad(zml.Shape.init(.{ .token = padding_tokens, .d = x.dim(-1) }, x.dtype()));
        flat_x = zml.Tensor.concatenate(&.{ flat_x, padding }, .token);
    }

    const blocked_x = flat_x
        .splitAxis(.token, .{ .token_block = .auto, .token_chunk = 128 })
        .splitAxis(.d, .{ .d_block = .auto, .d_chunk = 128 });
    const blocked_weight = linear.weight
        .splitAxis(.dout, .{ .dout_block = .auto, .dout_chunk = 128 })
        .splitAxis(.d, .{ .d_block = .auto, .d_chunk = 128 });
    ...
    var y = dotDequantizeFp8(blocked_x, blocked_weight, blocked_scale)
        .merge(.{ .token = .{ .token_block, .token_chunk }, .dout = .{ .dout_block, .dout_chunk } });

    if (padding_tokens > 0) {
        y = y.slice1d(.token, .{ .start = 0, .end = num_tokens });
    }
