...
pub const GatedDeltaNet = struct {
    ...
    pub fn forward(self: GatedDeltaNet, x: zml.Tensor, cache: KvCache.GatedDeltaNetCache, token_index: zml.Tensor) struct { zml.Tensor, KvCache.GatedDeltaNetCache } {
        ...
        const conv_q_input = if (use_cached_state)
            zml.Tensor.concatenate(&.{ cache.convQState(), projected_qkv.query }, .s)
        else
            projected_qkv.query;
        const conv_k_input = if (use_cached_state)
            zml.Tensor.concatenate(&.{ cache.convKState(), projected_qkv.key }, .s)
        else
            projected_qkv.key;
        const conv_v_input = if (use_cached_state)
            zml.Tensor.concatenate(&.{ cache.convVState(), projected_qkv.value }, .s)
        else
            projected_qkv.value;
        ...
        const updated_cache = cache.update(
            if (use_cached_state) buildUpdatedConvState(conv_q_input, left_pad) else buildUpdatedConvStateFromPrefix(projected_qkv.query, left_pad, token_index),
            if (use_cached_state) buildUpdatedConvState(conv_k_input, left_pad) else buildUpdatedConvStateFromPrefix(projected_qkv.key, left_pad, token_index),
            if (use_cached_state) buildUpdatedConvState(conv_v_input, left_pad) else buildUpdatedConvStateFromPrefix(projected_qkv.value, left_pad, token_index),
...
fn registerLeadingAxisAliases(
    ...
    const source = registry.tensors.get(source_key) orelse {
        log.err("Unable to create Qwen3.5 MoE GDN tensor aliases: missing source tensor {s}", .{source_key});
