...
pub const CompiledModel = struct {
    ...
    pub fn init(
        ...
        const prefill_exe = try compileKernel(allocator, io, platform, qwen_model, parameters, parameters.seqlen, parameters.prefill_moe_metadata, "prefill", progress);
        var prefill = try KernelExe.Runner.init(prefill_exe, allocator);
        errdefer prefill.deinit(allocator);
        const decode_exe = try compileKernel(allocator, io, platform, qwen_model, parameters, 1, parameters.decode_moe_metadata, "decode", progress);
        return .{
            .allocator = allocator,
...
pub fn run(runner: *KernelExe.Runner, args: RunArgs) void {
    var hidden_buffer: zml.Buffer = undefined;
    runner.run(.embed, args.io, .{
        .inputs = .{
            .embedding = args.model_buffers.text_model.embed_tokens,
    ...
    for (args.model_buffers.text_model.layers, args.layer_index_buffers) |layer_buffers, layer_index_buffer| {
        switch (layer_index_buffer) {
            .self_attn => |index_buffer| {
    ...
    runner.run(.sample, args.io, .{
        .inputs = .{
            .sampler = .{
                ...
                .lm_head = args.model_buffers.text_model.lm_head,
            },
            .hidden = hidden_buffer,
...
fn compileEmbed(allocator: std.mem.Allocator, io: std.Io, platform: *const zml.Platform, mdl: model.Model, parameters: CompilationParameters, seqlen: usize, phase: []const u8, progress: *std.Progress.Node) !zml.Exe {
...
fn compileFullAttention(allocator: std.mem.Allocator, io: std.Io, platform: *const zml.Platform, mdl: model.Model, parameters: CompilationParameters, seqlen: usize, layer_index: usize, moe_metadata: zml.moe.Metadata, phase: []const u8, progress: *std.Progress.Node) !zml.Exe {
...
fn compileLinearAttention(allocator: std.mem.Allocator, io: std.Io, platform: *const zml.Platform, mdl: model.Model, parameters: CompilationParameters, seqlen: usize, layer_index: usize, moe_metadata: zml.moe.Metadata, phase: []const u8, progress: *std.Progress.Node) !zml.Exe {
...
fn compileSample(allocator: std.mem.Allocator, io: std.Io, platform: *const zml.Platform, mdl: model.Model, parameters: CompilationParameters, seqlen: usize, phase: []const u8, progress: *std.Progress.Node) !zml.Exe {
...
fn compileExe(
    ...
    return platform.compileFn(allocator, io, function, args, .{
