...
fn testImplementation(
    ...
    //try zml.testing.testLayer(platform, activations, "model", qwen_model, qwen_weights, 1e-1);
    inline for (0..24) |i| {
        const name = std.fmt.comptimePrint("model.visual.blocks.{d}", .{i});
        //zml.testing.testLayer(platform, activations, name, qwen_model.vision_transformer.blocks[i], qwen_weights.vision_transformer.blocks[i], 1e-3); // pas de poids ici meilleure precision on peurt en deduire peut etre problee de conversion sur le reste
