...
pub fn main(init: std.process.Init) !void {
    ...
    const inputs_path = try common.writeEmbeddedSafetensors(allocator, io, inputs_bytes, "prefill_attention_inputs.safetensors");
    defer allocator.free(inputs_path);
    var registry: zml.safetensors.TensorRegistry = try .fromPath(allocator, io, inputs_path);
    defer registry.deinit();

    const outputs_path = try common.writeEmbeddedSafetensors(allocator, io, outputs_bytes, "prefill_attention_output.safetensors");
    ...
    var q = try common.loadBufferFromRegistry(allocator, io, platform, &registry, "query");
    defer q.deinit();
    var k = try common.loadBufferFromRegistry(allocator, io, platform, &registry, "key");
    defer k.deinit();
    var v = try common.loadBufferFromRegistry(allocator, io, platform, &registry, "value");
    defer v.deinit();
    var b_start_loc = try common.loadBufferFromRegistry(allocator, io, platform, &registry, "b_start_loc");
    defer b_start_loc.deinit();
    var b_seq_len = try common.loadBufferFromRegistry(allocator, io, platform, &registry, "b_seq_len");
