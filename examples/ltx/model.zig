...
/// Attention mode for block forward dispatch.
/// Controls whether attention runs in f32 (vanilla SDPA), bf16 without a custom kernel,
/// bf16 with CUDA Flash Attention 3 (SM90+), or bf16 with the ROCm Triton flash-attention kernel.
pub const AttnMode = enum {
    fp32_vanilla,
    bf16_sdpa,
    bf16_cuda_fa3,
    bf16_rocm_triton,
...
pub const Attention = struct {
    ...
    fn forwardImpl(self: Attention, comptime bf16_native: bool, x: Tensor, params: Params, num_heads: usize, opts: ForwardOpts) Tensor {
        ...
        var attn = if (bf16_native and opts.attn_meta != null and opts.attn_params != null)
            attention.fullSequenceAttention(qh, kh, vh, opts.attn_meta.?, opts.attn_params.?) // FA3 or ROCm Triton
        else if (bf16_native)
            sdpaNoF32Upcast(
                qh.rename(.{ .b = .batch }),
                kh.rename(.{ .b = .batch }),
                vh.rename(.{ .b = .batch }),
                .{},
            ).rename(.{ .batch = .b }) // bf16 chunked SDPA (A100, etc.)
        else
            zml.nn.sdpa(
                qh.rename(.{ .b = .batch }),
                kh.rename(.{ .b = .batch }),
                vh.rename(.{ .b = .batch }),
                .{},
            ).rename(.{ .batch = .b }); // f32 vanilla[B, Q, H, HD]
...
/// Helper: build SharedInputs from flat tensor args with attention mode applied.
fn buildSharedInputs(
    comptime attn_mode: AttnMode,
    ...
    v2a_k_pe_cos: Tensor,
    v2a_k_pe_sin: Tensor,
) BasicAVTransformerBlock.SharedInputs {
    return .{
        ...
        .video_attn_meta = switch (attn_mode) {
            .bf16_rocm_triton => .{ .rocm_triton_fa = {} },
            else => null,
        },
        .audio_attn_meta = switch (attn_mode) {
            .bf16_rocm_triton => .{ .rocm_triton_fa = {} },
            else => null,
        },
        .attn_params = switch (attn_mode) {
            .bf16_rocm_triton => .{ .rocm_triton_fa = .{} },
            else => null,
        },
    };
...
pub fn ForwardBlock0(comptime attn_mode: AttnMode, comptime is_stg: bool) type {
    return struct {
        ...
        fn doForward(vx_in: Tensor, ax_in: Tensor, inputs: BasicAVTransformerBlock.SharedInputs, params: Block0FullParams) BasicAVTransformerBlock.FullOutputs {
            ...
            return block.forwardNativeImpl(false, false, false, use_bf16, is_stg, is_stg, vx_in, ax_in, inputs, params);
        }
