/// M5-B parity checker: block-0 AV cross-attn V->A branch.
///
/// Verifies:
///   1) video_to_audio_attn(x, context, pe, k_pe) == attn_out
...
pub fn main(init: std.process.Init) !void {
    ...
    std.log.info("M5-B block0 AV V->A parity checker", .{});

    var it = init.minimal.args.iterate();
    _ = it.next();

    const ckpt_path = it.next() orelse {
        std.log.err("Usage: block0_av_v2a_check <checkpoint.safetensors> <fixture.safetensors> [token_limit]", .{});
    ...
    var x_buf = try loadBuf(allocator, io, platform, &fix_store, "block0_av_v2a.x", sharding);
    defer x_buf.deinit();
    var context_buf = try loadBuf(allocator, io, platform, &fix_store, "block0_av_v2a.context", sharding);
    defer context_buf.deinit();
    var pe_cos_buf = try loadBuf(allocator, io, platform, &fix_store, "block0_av_v2a.pe_cos", sharding);
    defer pe_cos_buf.deinit();
    var pe_sin_buf = try loadBuf(allocator, io, platform, &fix_store, "block0_av_v2a.pe_sin", sharding);
    defer pe_sin_buf.deinit();
    var k_pe_cos_buf = try loadBuf(allocator, io, platform, &fix_store, "block0_av_v2a.k_pe_cos", sharding);
    defer k_pe_cos_buf.deinit();
    var k_pe_sin_buf = try loadBuf(allocator, io, platform, &fix_store, "block0_av_v2a.k_pe_sin", sharding);
    defer k_pe_sin_buf.deinit();
    var attn_ref_buf = try loadBuf(allocator, io, platform, &fix_store, "block0_av_v2a.attn_out", sharding);
    defer attn_ref_buf.deinit();
    var gate_buf = try loadBuf(allocator, io, platform, &fix_store, "block0_av_v2a.gate", sharding);
    defer gate_buf.deinit();
    var mask_buf = try loadBuf(allocator, io, platform, &fix_store, "block0_av_v2a.mask", sharding);
    defer mask_buf.deinit();
    var delta_ref_buf = try loadBuf(allocator, io, platform, &fix_store, "block0_av_v2a.delta", sharding);
    ...
    var attn_shape = model.initBlock0AttentionParams(ckpt_store.view(), .video_to_audio_attn);

    std.log.info("Compiling V->A attention graph...", .{});
    var attn_exe = try platform.compileFn(
        allocator,
        io,
        model.forwardBlock0VideoToAudioAttnWithContextPeKPe,
        .{
    ...
    std.log.info("Executing V->A attention...", .{});
    ...
    std.log.info("✓ M5-B video_to_audio_attn parity PASSED", .{});

    std.log.info("Compiling V->A gated delta graph...", .{});
    var delta_exe = try platform.compileFn(
        allocator,
        io,
        model.forwardBlock0V2ADeltaFromAttnOut,
        .{
    ...
    std.log.info("Executing V->A gated delta algebra...", .{});
    ...
    std.log.info("✓ M5-B gated delta parity PASSED", .{});
    std.log.info("M5-B block0 AV V->A parity PASSED", .{});
}
