...
def main() -> None:
    ...
    # Use attn2 kwargs context (post prompt AdaLN modulation) for stream checks.
    v_text_ctx_raw = _as_tensor(mod_kwargs("attn2").get("context"))
    vgate_text_ca = aux("vgate_text_ca")
    ...
    # Use audio_attn2 kwargs context (post prompt AdaLN modulation) for stream checks.
    a_text_ctx_raw = _as_tensor(mod_kwargs("audio_attn2").get("context"))
    agate_text_ca = aux("agate_text_ca")
    ...
    add("block0_full.vgate_text_ca", vgate_text_ca)
    ...
    add("block0_full.agate_text_ca", agate_text_ca)
