"""Export M5-B fixture: block-0 AV cross-attn V->A branch.

Required keys (attn parity):
  block0_av_v2a.x
  block0_av_v2a.context
  block0_av_v2a.pe_cos
  block0_av_v2a.pe_sin
  block0_av_v2a.k_pe_cos
  block0_av_v2a.k_pe_sin
  block0_av_v2a.attn_out
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Export M5-B block0 AV V->A fixture")
...
def main() -> None:
    ...
    attn_key = f"{block_key}.video_to_audio_attn"

    entry = acts.get(attn_key)
    if not isinstance(entry, dict):
        raise ValueError(f"Missing V->A attn entry: {attn_key}")
    ...
    gate = acts.get(f"{block_key}.__aux__.v2a_gate")
    mask = acts.get(f"{block_key}.__aux__.v2a_mask")
    delta = acts.get(f"{block_key}.__aux__.v2a_delta")
    ...
    if missing:
        raise ValueError(f"Missing required V->A keys: {missing}")
    ...
    tensors = {
        "block0_av_v2a.x": x,
        "block0_av_v2a.context": context,
        "block0_av_v2a.pe_cos": pe_cos,
        "block0_av_v2a.pe_sin": pe_sin,
        "block0_av_v2a.k_pe_cos": k_pe_cos,
        "block0_av_v2a.k_pe_sin": k_pe_sin,
        "block0_av_v2a.attn_out": attn_out,
        "block0_av_v2a.gate": gate,
        "block0_av_v2a.mask": mask,
        "block0_av_v2a.delta": delta,
    }
