...
pub fn main(init: std.process.Init) !void {
    ...
    const stage2_checkpoint_path = it.next() orelse {
        std.log.err("Usage: bazel run //examples/ltx:ff_linear2_check -- <stage2_checkpoint.safetensors> <linear2_fixture.safetensors> [token_limit]", .{});
    ...
    var fixture_registry: zml.safetensors.TensorRegistry = zml.safetensors.TensorRegistry.fromPath(allocator, io, fixture_path) catch |err| {
        std.log.err("Failed to open linear2 fixture: {s}", .{fixture_path});
        return err;
    };
    ...
    std.log.info("Compiling FF linear2 graph...", .{});
    ...
    std.log.info("Executing FF linear2 forward...", .{});
    ...
    std.log.info("FF linear2 parity PASSED", .{});
}
