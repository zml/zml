...
const qwen3_inference = @import("qwen3/qwen3_inference.zig");
const qwen3 = @import("qwen3/qwen3_model.zig");
...
const Args = struct {
    ...
    pub const help =
        \\Options:
        \\  --model=<path>          Path to the DSpark model repository
        \\  --target-model=<path>   Path to the target Qwen3 model repository
...
pub fn main(init: std.process.Init) !void {
    ...
    defer qwen3.KvCache.deinitBuffer(&target_kv_cache_buffers);
    ...
    defer dspark.KvCache.deinitBuffer(&draft_kv_cache_buffers);
...
const Project = struct {
    ...
    parsed_target_config: std.json.Parsed(qwen3.Config),
...
fn initProject(init: std.process.Init, args: Args) !*Project {
    ...
    project.parsed_config = try parseConfig(dspark.Config, allocator, project.io, project.repo);
    errdefer project.parsed_config.deinit();
    project.parsed_target_config = try parseConfig(qwen3.Config, allocator, project.io, project.target_repo);
    ...
    project.tokenizer = try qwen3.loadTokenizer(allocator, project.io, project.target_repo);
...
fn tokenizePrompt(project: *Project, args: Args) !TokenizedPrompt {
    const token_ids = try qwen3.tokenizePrompt(project.allocator, &project.tokenizer, project.parsed_target_config.value, args.prompt);
    ...
    if (token_ids.len > prefill_seq_len) {
        std.debug.panic("prompt token count {} exceeds DSpark prefill length {}", .{ token_ids.len, prefill_seq_len });
    }
...
const ModelsAndCaches = struct {
    draft_model: dspark.Model,
    target_model: qwen3.Model,
    ...
    target_kv_cache: qwen3.KvCache,
    draft_kv_cache: dspark.KvCache,
    rng: zml.Tensor.Rng,
    sampling: qwen3.SamplingConfig,
    target_attention: qwen3_inference.TargetAttention,
    target_layers: qwen3_inference.TargetLayers,
    block_size: u32,
...
fn initModelsAndCaches(project: *Project, prompt: TokenizedPrompt, args: Args) !ModelsAndCaches {
    const draft_model = try dspark.Model.init(project.allocator, project.store.view(), project.parsed_config.value);
    errdefer draft_model.deinit(project.allocator);

    const target_model = try qwen3.Model.init(project.allocator, project.target_store.view(), project.parsed_target_config.value);
    ...
    const target_attention = qwen3_inference.TargetAttention.init(project.platform, cache_seq_len, project.parsed_target_config.value.num_attention_heads);
    ...
    return .{
        ...
        .target_layers = qwen3_inference.TargetLayers.init(draft_model.target_layer_ids),
...
fn compile(allocator: std.mem.Allocator, project: *Project, models: ModelsAndCaches, prompt: TokenizedPrompt) !CompiledExes {
    ...
    const target_prefill_exe = try qwen3_inference.compileTargetPrefill(
        allocator,
        project.io,
        project.platform,
        models.target_model,
    ...
    log.info("Compiling DSpark prefill drafter...", .{});
    const prefill_target_hidden_tensor = qwen3_inference.targetHiddenTensor(models.target_model, project.parsed_target_config.value, models.target_layers, prefill_seq_len);
    ...
    log.info("Compiling DSpark steady-state drafter...", .{});
    const draft_target_hidden_tensor = qwen3_inference.targetHiddenTensor(models.target_model, project.parsed_target_config.value, models.target_layers, models.block_size);
    ...
    const target_verify_exe = try qwen3_inference.compileTargetVerify(
        allocator,
        project.io,
        project.platform,
        models.target_model,
...
const LoadedBuffers = struct {
    model_buffers: dspark.Buffers,
    target_buffers: qwen3.Buffers,

    pub fn deinit(self: *LoadedBuffers, allocator: std.mem.Allocator) void {
        qwen3.Model.unloadBuffers(&self.target_buffers, allocator);
        dspark.Model.unloadBuffers(&self.model_buffers, allocator);
    }
};
...
fn runPrefill(
    ...
    target_kv_cache_buffers: *qwen3.KvCache.Buffer,
    ...
    const target_hidden_buffer, var target_token_buffer, var prefilled_target_kv_cache_buffers, var updated_rng_buffer = prefill_results.get(struct {
        zml.Buffer,
        zml.Buffer,
        qwen3.KvCache.Buffer,
        zml.Tensor.Rng.Buffer,
    });
    defer target_token_buffer.deinit();
    qwen3.KvCache.replaceBuffers(target_kv_cache_buffers, &prefilled_target_kv_cache_buffers);
...
fn runVerification(
    ...
    qwen3.KvCache.replaceBuffers(target_kv_cache_buffers, &verified_target_kv_cache_buffers);
    replaceRngBuffer(rng_buffer, &updated_rng_buffer);
...
fn runSpeculativeDecoding(
    ...
    target_kv_cache_buffers: *qwen3.KvCache.Buffer,
    draft_kv_cache_buffers: *dspark.KvCache.Buffer,
    rng_buffer: *zml.Tensor.Rng.Buffer,
    attention_metadata_buffers: anytype,
    ...
    while (start < prompt.max_seq_len and !stopped_on_eos) : (step += 1) {
        ...
        stdx.debug.assert(context_len >= 1 and context_len <= max_context_len, "invalid DSpark context length {}", .{context_len});

        @memset(block_tokens, models.draft_model.mask_token_id);
        ...
        var draft_token_buffer, var draft_logits_buffer, var updated_draft_kv_cache_buffers, var updated_rng_buffer = draft_exe.results.get(struct {
            zml.Buffer,
            zml.Buffer,
            dspark.KvCache.Buffer,
            zml.Tensor.Rng.Buffer,
        });
        defer draft_token_buffer.deinit();
        defer draft_logits_buffer.deinit();
        dspark.KvCache.replaceBuffers(draft_kv_cache_buffers, &updated_draft_kv_cache_buffers);
        replaceRngBuffer(rng_buffer, &updated_rng_buffer);
        ...
        for (block_tokens[0..@as(usize, @intCast(committed_tokens))]) |token| {
            if (qwen3.isEosToken(&project.parsed_target_config.value, token)) {
                stopped_on_eos = true;
                break;
            }
        ...
        if (!stopped_on_eos and start < prompt.max_seq_len) {
            if (qwen3.isEosToken(&project.parsed_target_config.value, correction_token)) {
                stopped_on_eos = true;
            } else {
...
fn draftTokens(
    ...
    rng: zml.Tensor.Rng,
    sampling: qwen3.SamplingConfig,
) struct { zml.Tensor, zml.Tensor, dspark.KvCache, zml.Tensor.Rng } {
    ...
    const hidden, const updated_kv_cache = draft_model.forward(target_hidden, block_tokens, position_ids, draft_kv_cache, cache_index, active_context_len);
    const sampled_tokens, const draft_logits, const updated_rng = draft_model.draftBlock(hidden, block_tokens, .{ .temperature = sampling.temperature }, rng);
