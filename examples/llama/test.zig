...
pub fn asyncMain() !void {
    ...
    const llama_options: llama_mod.LlamaOptions = .{
        ...
        .rope_opts = .{
            .impl = rope_impl,
            .freq_base = @floatCast(buffer_store.metadata("rope_freq_base", .float) orelse @as(f32, @floatFromInt(cli_args.rope_freq_base orelse 10_000))),
        },
    };
...
fn testImplementation(
    ...
    // try zml.testing.testLayer(platform, buffer_store, "model.model.embed_tokens", llama.model.embed_tokens, llama_weights.model.embed_tokens, 1e-3);
    // try zml.testing.testLayer(platform, buffer_store, "model.model.layers.0.self_attn.v_proj", llama.model.layers[0].self_attn.v_proj, llama_weights.model.layers[0].self_attn.v_proj, 1e-2);
    // try zml.testing.testLayer(platform, buffer_store, "model.model.layers.0.self_attn.q_proj", llama.model.layers[0].self_attn.q_proj, llama_weights.model.layers[0].self_attn.q_proj, 2e-2);
    // try zml.testing.testLayer(platform, buffer_store, "model.model.layers.0.self_attn.k_proj", llama.model.layers[0].self_attn.k_proj, llama_weights.model.layers[0].self_attn.k_proj, 2e-2);
    // try zml.testing.testLayer(platform, buffer_store, "model.model.layers.0.self_attn.o_proj", llama.model.layers[0].self_attn.o_proj, llama_weights.model.layers[0].self_attn.o_proj, 2e-2);
    // try zml.testing.testLayer(platform, buffer_store, "model.model.layers.0.mlp", llama.model.layers[0].mlp, llama_weights.model.layers[0].mlp, 1e-2);
    // try zml.testing.testLayer(platform, buffer_store, "model.model.layers.0.input_layernorm", llama.model.layers[0].input_layernorm, llama_weights.model.layers[0].input_layernorm, 1e-2);
    // try zml.testing.testLayer(platform, buffer_store, "model.model.layers.0.post_attention_layernorm", llama.model.layers[0].post_attention_layernorm, llama_weights.model.layers[0].post_attention_layernorm, 1e-2);

    {
        const test_case = "model.model.layers.0.self_attn";
        ...
        const SelfAttnPrefill = struct {
            ...
            // struct { Tensor, llama_mod.KvCache }
