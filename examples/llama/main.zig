...
pub fn generateText(
    ...
    const rng = try zml.Tensor.Rng.init(allocator, platform, seed);
    // defer rng.deinit(allocator);
    var generated_token_buffer = [_]u32{undefined};

    var prefill_res = prefill: {
        ...
        const PrefillArgs = stdx.meta.FnParam(@TypeOf(mod_prefill).call, 1);
        const args: PrefillArgs = try .initStatic(allocator);

        const prefill_tokens = try zml.Buffer.fromSlice(platform, .{max_seq_len}, prefill_buffer);
        args.set(args.handles[0], prefill_tokens);

        const prefill_token_pos = try zml.Buffer.scalar(platform, 0, .u32);
        defer prefill_token_pos.deinit();
        args.set(args.handles[1], prefill_token_pos);

        args.setSlice(args.handles[2].k, kv_cache_.buffers);
        args.setSlice(args.handles[3]._state, rng.buffers);

        // std.log.warn("prefill_args: {f}", .{args});
        const prefill_res = mod_prefill.call(args);
        // std.log.warn("prefill_res: {f}", .{prefill_res});
        _ = try prefill_res.buffers[0].toHost(std.mem.sliceAsBytes(prefill_buffer));
        generated_token_buffer[0] = prefill_buffer[prompt_tok.len - 1];
        break :prefill prefill_res;
    };
    // defer prefill_res.deinit(std.heap.smp_allocator);
    ...
    const GenArgs = stdx.meta.FnParam(@TypeOf(mod_generate).call, 1);
    const args: GenArgs = try .initStatic(allocator);
    // defer args.deinit(allocator);

    // args 0,1 are set in the loop
    args.setSlice(args.handles[2].k, prefill_res.getSlice(prefill_res.handles[1].k, zml.ops.staticCountTensors(llama.KvCache).?));
    args.set(args.handles[3]._state, prefill_res.get(prefill_res.handles[2]._state));
    ...
    generation: for (0..output_tokens_len + 1) |i| {
        ...
        args.set(args.handles[0], current_token);
        args.set(args.handles[1], token_pos);
        // call to generate the next token
        // std.log.warn("gen_args: {f}", .{args});
        const gen_res = mod_generate.call(args);
        // Note: we should pass gen_res to call so it's clear who owns what.
        defer gen_res.deinitStorage(std.heap.smp_allocator);

        // std.log.warn("gen_res: {f}", .{gen_res});
        // Donation invalidates the pjrt.Buffer handles, so we need to copy this manually.
        // It's kinda annoying. Could we do that as part of `call` ?
        current_token = gen_res.get(gen_res.handles[0]);
        args.setSlice(args.handles[2].k, gen_res.getField(KvCache, gen_res.handles[1]));
        args.setSlice(args.handles[3]._state, gen_res.getField(Tensor.Rng, gen_res.handles[2]));
...
pub fn asyncMain() !void {
    ...
    const kv_cache = try llama.KvCache.initBuffer(allocator, kv_shape, platform);
