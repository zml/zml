...
pub fn generateText(
    ...
    var double_buffered_token_buffer: [2]zml.Buffer = .{
        try .fromSlice(io, platform, generated_token_slice),
        undefined,
    };
    defer double_buffered_token_buffer[0].deinit();
    defer double_buffered_token_buffer[1].deinit();

    var double_buffered_token_pos_buffer: [2]zml.Buffer = undefined;
    var double_buffered_token_pos_buffer_initialized: [2]bool = .{ false, false };
    defer double_buffered_token_pos_buffer[0].deinit();
    defer double_buffered_token_pos_buffer[1].deinit();
    ...
    var current_input_index: usize = 0;
    ...
    generation: for (0..output_tokens_len + 1) |i| {
        ...
        if (double_buffered_token_pos_buffer_initialized[current_input_index]) {
            double_buffered_token_pos_buffer[current_input_index].deinit();
            double_buffered_token_pos_buffer_initialized[current_input_index] = true;
        }
        double_buffered_token_pos_buffer[current_input_index] = try .fromSliceOpts(io, platform, token_pos_slice, .{ .wait = false });

        // call to generate the next token
        decode_args.set(.{ double_buffered_token_buffer[current_input_index], double_buffered_token_pos_buffer[current_input_index], kv_cache_buffers, rng_buffers });

        decode_exe.call(decode_args, &decode_results);

        decode_results.fill(.{ &double_buffered_token_buffer[(current_input_index + 1) % 2], kv_cache_buffers, &rng_buffers });

        if (i != 0) {
            _ = try double_buffered_token_buffer[current_input_index].await(io);
            try double_buffered_token_buffer[current_input_index].toSlice(io, generated_token_slice);
        ...
        current_input_index = (current_input_index + 1) % 2;
    }
