...
const log = std.log.scoped(.dflash_gemma4);
...
fn validateArgs(args: Args) !void {
    if (args.model.len == 0 or args.target_model.len == 0) return error.MissingModelPath;
    if (args.prompt.len == 0) return error.EmptyPrompt;
    if (std.math.isNan(args.temperature) or args.temperature < 0) {
        return error.InvalidTemperature;
    }
}

fn tokenizePrompt(project: *Project, args: Args) !TokenizedPrompt {
    const token_ids = try project.chat_template.tokenizePrompt(project.allocator, &project.tokenizer, args.prompt);
    ...
    if (token_ids.len > prefill_seq_len) {
        log.err("prompt token count {} exceeds DFlash prefill length {}", .{ token_ids.len, prefill_seq_len });
...
fn compile(allocator: std.mem.Allocator, project: *Project, models: ModelsAndCaches, prompt: TokenizedPrompt) !CompiledExes {
    ...
    const prefill_target_hidden_tensor = gemma4_inference.targetHiddenTensor(models.target_model, models.target_model.config, models.target_layers, prefill_seq_len);
    ...
    const draft_target_hidden_tensor = gemma4_inference.targetHiddenTensor(models.target_model, models.target_model.config, models.target_layers, models.block_size);
...
fn runSpeculativeDecoding(
    ...
    while (start < prompt.max_seq_len and !stopped_on_eos) : (step += 1) {
        ...
        for (block_tokens[0..@as(usize, @intCast(committed_tokens))]) |token| {
            if (gemma4.isEosToken(&models.target_model.config, token)) {
                stopped_on_eos = true;
                break;
            }
        ...
        if (!stopped_on_eos and start < prompt.max_seq_len) {
            if (gemma4.isEosToken(&models.target_model.config, correction_token)) {
                stopped_on_eos = true;
            } else {
