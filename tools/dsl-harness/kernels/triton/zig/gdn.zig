...
// =============================================================================
// Harness wiring — re-export Kernel + declare SWEEPS + the XLA-pipeline driver.
// Multi-output kernel: `forward` returns `struct { Tensor, Tensor }` for
// (o, ht). zml.module.compile handles tuple-returning forwards via
// std.meta.ArgsTuple introspection on the function type.
// =============================================================================

pub const Kernel = FusedRecurrentGatedDeltaRule.Kernel;

pub const SWEEPS: []const harness.Sweep(Kernel.Config) = &.{
    // Mirrors `examples/triton_emitter/kernels_zig.zig` lines 293-312:
    // monorepo's `compile_gated_delta_net_kernel` config for qwen3_5
    // (num_tokens=64, num_qk_heads=4, num_v_heads=16, key_dim=32,
    // value_dim=64, num_sequences=2 → BK=32, BV=8).
    .{ .label = "default", .cfg = .{
...
/// Multi-output forward — returns (o, ht) as a 2-tuple. Mirrors
/// `examples/triton_emitter/dump_via_xla.zig:FusedRecurrentGatedDeltaRule`.
pub fn forward(
    q: Tensor, k: Tensor, v: Tensor, g: Tensor, beta: Tensor, h0: Tensor, cu_seqlens: Tensor,
    _: Tensor, _: Tensor,
...
pub fn args() std.meta.ArgsTuple(@TypeOf(forward)) {
    // Shape budget matches the cfg in the default sweep:
    // num_tokens=64, num_qk_heads=4, num_v_heads=16, key_dim=32,
    // value_dim=64, num_sequences=2.
    return .{
        Tensor.init(.{1 * 64 * 4 * 32}, .bf16),    // q_ptr      (1, T, H, K)
        Tensor.init(.{1 * 64 * 4 * 32}, .bf16),    // k_ptr      (1, T, H, K)
        Tensor.init(.{1 * 64 * 16 * 64}, .bf16),   // v_ptr      (1, T, HV, V)
        Tensor.init(.{1 * 64 * 16}, .f32),         // g_ptr      (1, T, HV)
        Tensor.init(.{1 * 64 * 16}, .bf16),        // beta_ptr   (1, T, HV)
        Tensor.init(.{2 * 16 * 32 * 64}, .f32),    // h0_ptr     (NS, HV, K, V)
        Tensor.init(.{3}, .i32),                    // cu_seqlens_ptr (NS+1,)
        Tensor.init(.{1 * 64 * 16 * 64}, .bf16),   // o_ptr      (output)
        Tensor.init(.{2 * 16 * 32 * 64}, .f32),    // ht_ptr     (output)
    };
}
