# Vendored from JAX `platforms/tpu/kernel.py` — Apache 2.0.
# Slim cut (no `ref_*`, no `dynamic_validate_inputs`, no auto-pick block
# sizes) plus a `build_args(cfg)` adapter and standalone `main()` for the
# harness.
...
def ragged_paged_attention(
    ...
    Block sizes (`num_kv_pages_per_block`, `num_queries_per_block`) are required
    here — the upstream auto-tuner is intentionally not vendored.
    """
...
# ---------------------------------------------------------------------------
# Harness registration adapter — ZML-specific, called by the mosaic_tpu runner.
# ---------------------------------------------------------------------------

from typing import Any, Dict, Tuple

__all__ = ["ragged_paged_attention", "build_args"]
...
def build_args(cfg: Dict[str, Any]) -> Tuple[list, Dict[str, Any]]:
    ...
    q_dtype = _dtype(cfg.get("q_dtype"), jnp.bfloat16)
    kv_dtype = _dtype(cfg.get("kv_dtype"), jnp.bfloat16)
    num_q_tokens = int(cfg.get("num_q_tokens", 8))
    num_q_heads = int(cfg.get("num_q_heads", 8))
    num_kv_heads = int(cfg.get("num_kv_heads", 2))
    total_num_pages = int(cfg.get("total_num_pages", 8))
    page_size = int(cfg.get("page_size", 16))
    max_num_seqs = int(cfg.get("max_num_seqs", 2))
    pages_per_seq = int(cfg.get("pages_per_seq", 2))

    q = jax.ShapeDtypeStruct((num_q_tokens, num_q_heads, head_dim), q_dtype)
    kv_pages = jax.ShapeDtypeStruct(
        (total_num_pages, page_size, num_kv_heads * 2, head_dim), kv_dtype
    )
    ...
    # Optional knobs: missing JSON keys → None, which Pallas treats as off.
    static_kwargs = {
        "sm_scale": float(cfg.get("sm_scale", 0.0883883461)),
        "num_kv_pages_per_block": int(cfg.get("num_kv_pages_per_block", 1)),
        "num_queries_per_block": int(cfg.get("num_queries_per_block", 8)),
        "sliding_window": cfg.get("sliding_window"),
        "soft_cap": cfg.get("soft_cap"),
        "k_scale": cfg.get("k_scale"),
        "v_scale": cfg.get("v_scale"),
    }
    return args, static_kwargs


# ---------------------------------------------------------------------------
# Standalone CLI for direct lowering verification (run: `python ragged_paged.py`).
# ---------------------------------------------------------------------------
...
MAX_NUM_SEQS = 2
PAGES_PER_SEQ = 2
SM_SCALE = 0.0883883461  # 1/sqrt(128)
...
def main() -> None:
    # Abstract-TPU mesh + CPU-lowering shim mirror what
    # `platforms/tpu/gen_ir.py` does — lets us lower the kernel from a
    # non-TPU host and still print the Mosaic module.
    import argparse
    import contextlib
    import os
    import sys
    ...
    # Positional fall-throughs preserve the original `kernel.py HEAD_DIM KV_DTYPE`
    # invocation used by older harness runs.
    parser.add_argument("head_dim_pos", nargs="?", type=int, default=None)
    parser.add_argument("kv_dtype_pos", nargs="?", default=None)
    parser.add_argument("--head-dim", type=int, default=HEAD_DIM)
    parser.add_argument("--kv-dtype", default="bf16")
    parser.add_argument("--q-dtype", default="bf16")
    parser.add_argument("--num-q-tokens", type=int, default=NUM_Q_TOKENS)
    parser.add_argument("--num-q-heads", type=int, default=NUM_Q_HEADS)
    parser.add_argument("--num-kv-heads", type=int, default=NUM_KV_HEADS)
    parser.add_argument("--total-num-pages", type=int, default=TOTAL_NUM_PAGES)
    parser.add_argument("--page-size", type=int, default=PAGE_SIZE)
    parser.add_argument("--max-num-seqs", type=int, default=MAX_NUM_SEQS)
    parser.add_argument("--pages-per-seq", type=int, default=PAGES_PER_SEQ)
    parser.add_argument(
            "--num-kv-pages-per-block", type=int, default=NUM_KV_PAGES_PER_BLOCK
    )
    parser.add_argument(
            "--num-queries-per-block", type=int, default=NUM_QUERIES_PER_BLOCK
    )
    parser.add_argument("--sm-scale", type=float, default=SM_SCALE)
    parser.add_argument("--sliding-window", default=None)
    parser.add_argument("--soft-cap", default=None)
    parser.add_argument("--k-scale", default=None)
    parser.add_argument("--v-scale", default=None)
    args = parser.parse_args()

    head_dim = args.head_dim_pos if args.head_dim_pos is not None else args.head_dim
    kv_dtype_name = args.kv_dtype_pos if args.kv_dtype_pos is not None else args.kv_dtype
    q_dtype_name = args.q_dtype
    kv_dtype = _DTYPE_MAP[kv_dtype_name]
    q_dtype = _DTYPE_MAP[q_dtype_name]
    sliding_window = _parse_optional_int(args.sliding_window)
    soft_cap = _parse_optional_float(args.soft_cap)
    k_scale = _parse_optional_float(args.k_scale)
    v_scale = _parse_optional_float(args.v_scale)
    ...
    q = jax.ShapeDtypeStruct(
            (args.num_q_tokens, args.num_q_heads, head_dim), q_dtype
    )
    kv_pages = jax.ShapeDtypeStruct(
            (args.total_num_pages, args.page_size, args.num_kv_heads * 2, head_dim),
            kv_dtype,
    )
    kv_lens = jax.ShapeDtypeStruct((args.max_num_seqs,), jnp.int32)
    page_indices = jax.ShapeDtypeStruct(
            (args.max_num_seqs, args.pages_per_seq), jnp.int32
    )
    cu_q_lens = jax.ShapeDtypeStruct((args.max_num_seqs + 1,), jnp.int32)
    num_seqs = jax.ShapeDtypeStruct((1,), jnp.int32)
