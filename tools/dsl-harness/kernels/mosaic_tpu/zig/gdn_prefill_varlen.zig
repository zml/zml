...
pub const Kernel = zml.kernel.mosaic_tpu.Kernel(Cfg, .{
    ...
    .inputs = &.{ "h_v", "c", "seq_id", "is_first", "is_last", "q", "k", "v", "g_cum", "beta", "s_in" },
    .outputs = &.{ "o", "s_out" },
    .run = run,
});

const baseline = Cfg{
    // Single-seq, no GVA, BT=64, T=128 (2 chunks).
    .num_k_heads = 8,
    .num_v_heads = 8,
    .head_k_dim = 128,
    .head_v_dim = 128,
...
pub const SWEEPS: []const harness.Sweep(Cfg) = &.{
    .{ .label = "single_seq_no_gva", .cfg = baseline },
    .{ .label = "qwen35_single_seq", .cfg = override(.{
        .num_k_heads = 16,
        .num_v_heads = 32,
        .seqlens = .{ 256, 0, 0, 0, 0, 0, 0, 0 },
    }) },
    .{ .label = "multi_seq", .cfg = override(.{
        .num_seqs = 4,
        .seqlens = .{ 256, 192, 128, 64, 0, 0, 0, 0 },
    }) },
};
