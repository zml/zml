...
fn parseExpertKey(name: []const u8) ?ExpertKey {
    ...
    if (!(std.mem.eql(u8, kind, "weight") or
        std.mem.eql(u8, kind, "weight_scale") or
        std.mem.eql(u8, kind, "weight_scale_2") or
        std.mem.eql(u8, kind, "input_scale"))) return null;
    const marker = std.mem.indexOf(u8, name, ".experts.") orelse return null;
    const leaf_start = marker + ".experts.".len + expert_text.len + 1;
    return .{ .prefix = name[0 .. marker + ".experts".len], .expert = expert, .leaf = name[leaf_start..] };
}
...
fn rewriteShard(allocator: std.mem.Allocator, io: std.Io, path: []const u8, keys: []const []const u8, expected_experts: ?usize, buffer_size: usize, verbose: bool) !struct { std.StringArrayHashMapUnmanaged([]const u8), u64 } {
    ...
    for (groups.values()) |group| {
        ...
        if (is_w1) {
            ...
            const shape_len = if (sources[0].shape.len == 0) 2 else sources[0].shape.len + 1;
            const shape = try arena.alloc(i64, shape_len);
            shape[0] = @intCast(count);
            if (sources[0].shape.len == 0) {
                // Per-tensor NVFP4 scales are paired as [expert, w1_or_w3].
                shape[1] = 2;
            } else {
            ...
            try outputs.append(arena, .{
                .name = try std.fmt.allocPrint(arena, "{s}.w13.{s}", .{ group.prefix, suffix }),
                .dtype = sources[0].dtype,
                .shape = shape,
                .sources = sources,
                .interleave = sources[0].shape.len != 0,
            });
...
pub fn main(init: std.process.Init) !void {
    ...
    while (args.next()) |arg| {
        if (std.mem.eql(u8, arg, "--input-dir")) input_dir = args.next() orelse return error.MissingArgument else if (std.mem.eql(u8, arg, "--num-experts")) expected_experts = try std.fmt.parseInt(usize, args.next() orelse return error.MissingArgument, 10) else if (std.mem.eql(u8, arg, "--buffer-size-mb")) buffer_mb = try std.fmt.parseInt(usize, args.next() orelse return error.MissingArgument, 10) else if (std.mem.eql(u8, arg, "--verbose")) verbose = true else if (std.mem.eql(u8, arg, "--help")) {
            std.debug.print("usage: repack_nvfp4 [--input-dir DIR] [--num-experts N] [--buffer-size-mb N] [--verbose]\nRewrites an NVFP4 DeepSeek V4 sharded safetensors checkpoint in place.\n", .{});
            return;
