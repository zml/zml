...
pub const ck = @import("fp8/ck.zig");
...
pub const Backend = enum {
    ...
    pub fn selected() Backend {
        const value = std.mem.span(std.c.getenv("ZML_FP8_BACKEND") orelse return .triton);
        if (std.mem.eql(u8, value, "triton")) return .triton;
        if (std.mem.eql(u8, value, "ck")) return .ck;
        stdx.debug.panic("ZML_FP8_BACKEND must be 'triton' or 'ck', got '{s}'", .{value});
    }
};
...
fn ckBlockScaledDotLocal(lhs: Tensor, rhs_fn: Tensor, rhs_scale_fn: Tensor, output_shape: Shape) Tensor {
    const prepared = prepareBlockScaledDot(lhs, rhs_fn, rhs_scale_fn);
    const out = ck.gemm(
        prepared.a,
        prepared.b,
        prepared.a_scale,
        prepared.b_scale,
        Shape.init(.{ .fp8_m = prepared.m, .fp8_n = prepared.n }, .bf16),
    );
    return out.reshape(output_shape);
}
...
fn blockScaledDot(
    ...
    return ops.manualComputation(
        ...
        output_shape,
        BlockDotContext{ .backend = backend, .reduce_partials = reduce_partials },
        (struct {
            fn body(ctx: BlockDotContext, _: std.mem.Allocator, inputs: []const Tensor, local_output: Shape) Tensor {
                const partial = switch (ctx.backend) {
                    .triton => tritonBlockScaledDotLocal(inputs[0], inputs[1], inputs[2], local_output),
                    .ck => ckBlockScaledDotLocal(inputs[0], inputs[1], inputs[2], local_output),
                };
...
pub fn tritonBlockScaledDot(lhs: Tensor, rhs_fn: Tensor, rhs_scale_fn: Tensor, output_shape: Shape) Tensor {
    return blockScaledDot(lhs, rhs_fn, rhs_scale_fn, output_shape, .triton);
}

pub fn ckBlockScaledDot(lhs: Tensor, rhs_fn: Tensor, rhs_scale_fn: Tensor, output_shape: Shape) Tensor {
    return blockScaledDot(lhs, rhs_fn, rhs_scale_fn, output_shape, .ck);
}
