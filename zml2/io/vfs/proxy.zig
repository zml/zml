...
pub const IoProxy = struct {
    inner: std.Io,

    pub fn init(io: std.Io) IoProxy {
        return .{ .inner = io };
    }

    pub fn partial(overrides: anytype) std.Io.VTable {
        var new_vtable = vtable;
    ...
    pub fn as(userdata: ?*anyopaque) *IoProxy {
    ...
    pub fn async(userdata: ?*anyopaque, result: []u8, result_alignment: std.mem.Alignment, context: []const u8, context_alignment: std.mem.Alignment, start: *const fn (context: *const anyopaque, result: *anyopaque) void) ?*std.Io.AnyFuture {
        const self: *IoProxy = @ptrCast(@alignCast(userdata orelse unreachable));
    ...
    pub fn groupAsync(userdata: ?*anyopaque, group: *std.Io.Group, context: []const u8, context_alignment: std.mem.Alignment, start: *const fn (*std.Io.Group, context: *const anyopaque) std.Io.Cancelable!void) void {
        const self: *IoProxy = @ptrCast(@alignCast(userdata orelse unreachable));
        return self.inner.vtable.groupAsync(self.inner.userdata, group, context, context_alignment, start);
    }

    pub fn groupConcurrent(userdata: ?*anyopaque, group: *std.Io.Group, context: []const u8, context_alignment: std.mem.Alignment, start: *const fn (*std.Io.Group, context: *const anyopaque) std.Io.Cancelable!void) std.Io.ConcurrentError!void {
        const self: *IoProxy = @ptrCast(@alignCast(userdata orelse unreachable));
