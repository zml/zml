...
const default_candidates = [_][]const u8{
    "/home/louis/flashinfer-gptoss-sm90-bf16-mxfp4-standalone/build_gptoss/libgptoss_moe_cutlass_sm90.s",
};
...
pub fn load(allocator: std.mem.Allocator) !void {
    ...
    if (try r.rlocation("home/louis/flashinfer-gptoss-sm90-bf16-mxfp4-standalone/build_gptoss/libgptoss_moe_cutlass_sm90.so", &buffer)) |so_path| {
    ...
    std.log.err(
        \\FlashInfer MoE backend requires libgptoss_moe_cutlass_sm90.so, but it was not found.
        \\Compile it from this repo: https://github.com/zml/moe_gemm_kernel_fi_sm90_bf16_mxfp4.git
        \\Set FLASHINFER_MOE_KERNEL_SO to the absolute .so path, or make it available in Bazel runfiles.
        \\Fallback path checked: {s}
    ,
