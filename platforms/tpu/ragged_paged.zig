...
pub const Cfg = struct {
    ...
    /// Pre-mask softcap: `qk = soft_cap * tanh(qk / soft_cap)`.
    soft_cap: ?f32 = null,
    /// K dequant scale applied right after `stridedLoadKv` (mirrors Pallas's
    /// `k.astype(f32) * k_scale → k.astype(q_dtype)`).
    k_scale: ?f32 = null,
    v_scale: ?f32 = null,
    ...
    /// Same packing rule applied to `q_dtype` — used by `fold_on_2nd_minor`
    /// to decide whether to cast q to f32 before the trailing-dim reshape.
    pub fn qPackingFactor(self: Cfg) i64 {
        return switch (self.q_dtype) {
            .f32 => 1,
            .f16, .bf16 => 2,
            else => std.debug.panic("ragged_paged: unsupported Q dtype {s}", .{@tagName(self.q_dtype)}),
        };
    }
...
fn flashAttention(
    ...
    kv_k: mtt.Value,
    v: mtt.Value,
    /// Element type of `kv_k` as currently held — equal to `cfg.kv_dtype`
    /// unless `cfg.k_scale` was applied upstream (then `cfg.q_dtype`).
    /// Used for the masked-zero round-trip Pallas's `.astype(k.dtype)` does.
    k_input_dtype: mtt.DType,
    /// Same role as `k_input_dtype`, for `v`.
    v_input_dtype: mtt.DType,
    kv_head_idx: i64,
    q_head_idx: i64,
    kv_blk_idx: mtt.Value,
    kv_len: mtt.Value,
    ...
    const k_zeroed = k.select(kv_mask, kv_k.to(.f32), k.zeros(&.{ flash_k, head_dim }, .f32)).to(k_input_dtype);
    const v_zeroed = k.select(kv_mask, v.to(.f32), k.zeros(&.{ flash_k, head_dim }, .f32)).to(v_input_dtype);
    ...
    k.traceStop();
    var qk = k.mulf(qk_raw, k.splat(cfg.sm_scale, &.{ flash_q, flash_k }, .f32));
    ...
    if (cfg.soft_cap) |sc| {
        ...
        qk = k.mulf(sc_splat, k.tanh(k.divf(qk, sc_splat)));
    }
...
pub fn run(b: *mtt.Builder, cfg: Cfg) mtt.FinishError!void {
        ...
        }
        {
            ...
            while (kv_head_chunk_idx < num_kv_heads_per_blk) : (kv_head_chunk_idx += kv_load_step) {
                ...
                while (step_idx < kv_load_step) : (step_idx += 1) {
                    var kv_k_val = kv_pairs[step_idx].k;
                    var kv_v_val = kv_pairs[step_idx].v;
                    var k_input_dtype = cfg.kv_dtype;
                    var v_input_dtype = cfg.kv_dtype;
                    if (cfg.k_scale) |ks| {
                        const ks_splat = k.splat(ks, &.{ cfg.flashKRows(), cfg.head_dim }, .f32);
                        kv_k_val = k.mulf(kv_k_val.to(.f32), ks_splat).to(cfg.q_dtype);
                        k_input_dtype = cfg.q_dtype;
                    }
                    if (cfg.v_scale) |vs| {
                        const vs_splat = k.splat(vs, &.{ cfg.flashKRows(), cfg.head_dim }, .f32);
                        kv_v_val = k.mulf(kv_v_val.to(.f32), vs_splat).to(cfg.q_dtype);
                        v_input_dtype = cfg.q_dtype;
                    }
                    const kv_head_idx: i64 = @intCast(kv_head_chunk_idx + step_idx);
                    const q_head_idx = kv_head_idx * cfg.numQHeadsPerKvHead();

                    // `fold_on_2nd_minor` (kernel.py:255-262): when the
                    // 2nd-to-last dim is not a multiple of q's packing
                    // factor, cast q to f32 before the trailing-dim reshape
                    // so the layout pass can fold it cleanly.
                    var q_block = k.vectorLoadShape(a.q, &.{ k.cIndex(0), k.cIndex(q_head_idx), k.cIndex(0) }, &.{ cfg.numQPerBlk(), cfg.numQHeadsPerKvHead(), cfg.head_dim });
                    if (@mod(cfg.numQHeadsPerKvHead(), cfg.qPackingFactor()) != 0) {
                        q_block = q_block.to(.f32);
                    }
                    const q_2d = k.shapeCast(q_block, &.{ cfg.flashQRows(), cfg.head_dim });
                    flashAttention(k, cfg, a, q_2d, kv_k_val, kv_v_val, k_input_dtype, v_input_dtype, kv_head_idx, q_head_idx, kv_blk, kv_len, q_start, q_end, q_len, q_len_start);
                }
            }
    ...
    k.vectorStoreAt(a.o, k.refLoad(a.acc_ref).to(cfg.q_dtype), &.{ k.cIndex(0), k.cIndex(0), k.cIndex(0) });
}

/// Open a builder, run the kernel body, and return the canonicalized MLIR IR.
/// Used by offline tooling (`examples/mosaic_ragged_paged`); production model
/// code goes through `zml.kernel.mosaic_tpu.Kernel(...).emit(...)` instead.
pub fn buildIr(allocator: std.mem.Allocator, ctx: *mlir.Context, cfg: Cfg) ![:0]const u8 {
    var b = try mtt.Builder.open(allocator, ctx, "ragged_paged_attention_kernel");
    defer b.deinit();
    try run(&b, cfg);
