...
// Parser for the ZML-owned backend_config carried by synthetic
// `zml$neuron$nki` custom-calls. This format is intentionally simple text
// because it is produced while emitting StableHLO and consumed only by the
// Neuron hook before neuronx-cc sees the module.
...
// Parse the inline kernel source, entrypoint, and tensor signatures attached to
// one synthetic NKI custom-call.
