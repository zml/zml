...
The Zig hook writes one JSON request per synthetic `zml$neuron$nki` custom-call.
This script rewrites the Python entrypoint names, asks NKI to produce BIR, then
returns the backend_config and IO names needed to mutate the surrounding HLO.
"""
...
def parse_request(raw_request: dict) -> CompileRequest:
    """Parse the request shape produced by nki_compiler_client.zig."""
...
def build_backend_config(
    ...
    """Build the AwsNeuronCustomNativeKernel backend_config payload."""
