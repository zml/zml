...
fn setupRocmEnv(rocm_data_dir: []const u8) !void {
    ...
    _ = c.setenv("ROCM_PATH", try stdx.fs.path.bufJoinZ(&buf, &.{rocm_data_dir}), 1); // need to be zero terminated
    _ = c.setenv("ROCBLAS_TENSILE_LIBPATH", try stdx.fs.path.bufJoinZ(&buf, &.{ rocm_data_dir, "lib", "rocblas", "library" }), 1);
    _ = c.setenv("HIPBLASLT_TENSILE_LIBPATH", try stdx.fs.path.bufJoinZ(&buf, &.{ rocm_data_dir, "lib", "hipblaslt", "library" }), 1);
    _ = c.setenv("HIPBLASLT_EXT_OP_LIBRARY_PATH", try stdx.fs.path.bufJoinZ(&buf, &.{ rocm_data_dir, "lib", "hipblaslt", "library", "hipblasltExtOpLibrary.dat" }), 1);
}
